...
from common_code.wsi.rankmix_teacher import (
    load_rankmix_teacher,
    train_rankmix_teacher,
)
from common_code.wsi.eval import _save_and_evaluate_bags
from common_code.wsi.progress import _progress_payload, _write_training_progress
from common_code.metrics.payload import resolve_device as _resolve_device
...
def _cache_dir(config: dict, seed: int) -> Path:
    wsi_training = config.get("wsi_training", {})
    bag_cache_dir = wsi_training.get("bag_cache_dir") or config.get("wsi_bag_cache_dir")
    if bag_cache_dir:
        return Path(str(bag_cache_dir))
    root = Path(str(config.get("output_root", ".")))
    return root / "data" / "wsi_bag_cache" / f"seed={seed}"
