...
\maketitle
\begin{abstract}
\noindent TCGA-UT patch classifiers trained on more patients are more accurate than redundancy-adjusted effective support predicts, and at a fixed patient count a cohort of mutually similar patients is 8.30 percentage points less accurate than a random cohort. Without training new classifiers, this analysis asks whether two cohort properties account for both findings: similarity between patients, added to effective support, and coverage, the distance from validation patients to their nearest training patient. On 135 random cohorts, adding coverage lowers the accuracy gained per doubling of patients beyond effective support from 2.62 to 0.79 points, but the 95\% interval of $-1.25$ to 2.82 points is wide because coverage and patient count change together. A relation fitted to these random cohorts predicts a concentration damage of 6.80 points against the observed 8.30, and a gain from coverage-maximizing selection of 2.10 points against the observed 1.04; both prediction errors exclude zero but cross the one-point threshold. The outcome is inconclusive. Coverage and similarity are compatible with the whole breadth benefit and with most of the concentration damage, but the random cohorts do not identify their contributions precisely.
\end{abstract}
