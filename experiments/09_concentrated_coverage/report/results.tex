...
\label{sec:follow-up}
A follow-up experiment would compare three allocations of twenty patients contributing eight patches each: the focal seed and its nineteen nearest neighbours (\emph{clustered}), nineteen randomly drawn additions (\emph{random}), or nineteen additions selected by facility location on the training pool (\emph{dispersed}). All three share the focal seed, patient count, patch budget, and patch-sampling rule. The random allocation distinguishes damage from deliberate concentration from improvement over ordinary sampling. Three splits and five draws require 45 fits, with the same logistic-regression readout and validation tuning as this experiment.
