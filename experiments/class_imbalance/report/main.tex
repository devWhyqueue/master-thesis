...
For each seed, the split is assigned at the TCGA participant level rather than at the slide level. Case identifiers are derived from the first three TCGA barcode fields, so all diagnostic slides from the same participant are kept in the same train, validation, or test partition. Each seed contains \num{5025} training cases, \num{1075} validation cases, and \num{1075} test cases. Because cases can contribute different numbers of diagnostic slides, slide counts vary across seeds: training contains \num{6091}--\num{6129} slides, validation \num{1276}--\num{1337} slides, and test \num{1299}--\num{1331} slides. The same case-level assignments are reused by both benchmarks, so no patches, frozen features, or WSI bags from the same slide or participant cross train, validation, and test.
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[
    ...
    family/.style={box, fill=blue!6, minimum width=2.55cm},
    method/.style={box, fill=white, minimum width=2.55cm},
    arrow/.style={-{Latex[length=2mm]}, thick}
  ]
    \node[baseline] (ce) at (0,0) {No-mitigation\\baseline\\CE / MIL CE};
    \node[family] (data) at (-6.0,-1.75) {Data-level\\sampling and augmentation};
    \node[method, below=0.38cm of data] (dataMethods) {Balanced sampling\\RankMix};
    \node[family] (loss) at (-3.0,-1.75) {Algorithm-level\\losses};
    \node[method, below=0.38cm of loss] (lossMethods) {Weighted CE\\Focal loss\\CFAL (patch)};
    \node[family] (hybrid) at (0,-1.75) {Hybrid\\sampling--loss};
    \node[method, below=0.38cm of hybrid] (hybridMethods) {CE + soft F1\\CE + soft MCC\\MDE-MIL (WSI)};
    \node[family] (synthetic) at (3.0,-1.75) {Synthetic data\\generation};
    \node[method, below=0.38cm of synthetic] (syntheticMethods) {ProGAN\\augmentation};
    \node[family] (repr) at (6.0,-1.75) {Representation and\\architecture};
...
The simpler patch interventions separate mainly by how little they move the validation-selected classifier after freezing Virchow2. Class-weighted CE, balanced sampling, focal loss, and ProGAN augmentation remain near CE on mean macro F1. ProGAN augmentation stays within the non-generative spread despite adding a large synthetic tail-class volume (Table~\ref{tab:progan-diagnostics}). The accompanying FID and Virchow2 nearest-neighbor diagnostics indicate that this is not merely a quantity problem: many generated patches are visually degenerate or near-monochrome and are distant from the real feature manifold used by the classifier (Figure~\ref{fig:progan-examples}). The small focal-loss shift is consistent with the interpretation that difficulty reweighting is not very helpful when most cancer types are already separable in the frozen embedding space.
...
\begin{figure}[htbp]
  ...
  \caption{Validation-selected WSI-bag macro F1 on the held-out test split by random seed. The dashed line marks plain MIL CE; seed-wise spread shows why cross-method gaps should be read cautiously when standard deviations overlap (Table~\ref{tab:wsi-results}).}
