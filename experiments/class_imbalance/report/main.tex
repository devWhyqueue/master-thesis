...
The number of odd classes $k$ controls set size and therefore the degree of logit averaging. The paper's ablation experiments show that accuracy and calibration are insensitive to the specific value of $k$, and the default of $k=1$ is the computationally cheapest option. The validation sweep covers $k\in\{1,2,3,4,5,6,8,12,20\}$ with the default fixed at $k=1$. The validation-selected configuration reported later is $k=4$ (Table~\ref{tab:tuning-selection}), indicating that a moderate set size works best in this benchmark without requiring the largest sets.
