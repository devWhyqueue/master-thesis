...
The number of odd classes $k$ controls set size and therefore the degree of logit averaging. The paper's ablation experiments show that accuracy and calibration are insensitive to the specific value of $k$, and the default of $k=1$ is the computationally cheapest option. The validation sweep covers $k\in\{1,4,8,20\}$ (representative subset; all integer values from 1 to 20 were evaluated) with the default fixed at $k=1$. Validation selects $k=8$: mean validation macro F1 rises from \num{0.776} at $k=1$ to \num{0.783} at $k=8$ and then falls to \num{0.768} at $k=20$, confirming that performance is robust across moderate set sizes but degrades when the set becomes large and computation dominates.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{@{}p{0.13\linewidth}p{0.25\linewidth}p{0.22\linewidth}X@{}}
    ...
    Patch & D\&C & $k=10$, bins$=5$ & bins$\in\{1,5,20\}$; expert epochs$\in\{5,20\}$\\
...
The patch benchmark compresses into a narrow performance band once patches are represented with frozen Virchow2 embeddings (Table~\ref{tab:patch-results}). CFAL ranks highest on macro F1 and balanced accuracy, with paired gains over plain CE on both headline metrics in all three seeds (Table~\ref{tab:paired-deltas}). OKO reaches the second-highest balanced accuracy---marginally below CFAL and above CE plus soft F1---and achieves a macro F1 between CE plus soft F1 and CE plus soft MCC. Together, CFAL, OKO, and the Scholz-style hybrid objectives form a second tier clearly above the CE baseline; within this group, CE plus soft F1 is slightly ahead of OKO on macro F1 while OKO leads on balanced accuracy. With the fixed support-tier partition, D\&C trails CE on both headline metrics.

The simpler patch interventions separate mainly by how little they move the validation-selected classifier after freezing Virchow2. Class-weighted CE, balanced sampling, focal loss, and ProGAN augmentation remain near CE on mean macro F1. ProGAN augmentation stays within the non-generative spread despite adding a large synthetic tail-class volume (Table~\ref{tab:progan-diagnostics}). The likely cause is the frozen-feature pipeline rather than poor generator quality. The class-specific generators reach an Inception FID of \num{140}--\num{390} (median \num{211}), which is in the same range as---and at the median better than---the ProGAN FID of \num{222}--\num{261} reported by Ruiz-Casado et al.\ on a binary breast-histology task where GAN augmentation still produced large downstream gains \citep{ruizcasado2024enhancingHistopathological}. The decisive difference is how the synthetic patches enter the classifier: the source method trains a convolutional classifier end to end on pixels, so it can adapt its features to the generated distribution, whereas this benchmark embeds every patch with a frozen Virchow2 encoder and trains only a lightweight head. A frozen encoder maps any synthetic patch that is even slightly off the real distribution into embedding regions the head never sees for real data, so additional synthetic volume cannot help the head regardless of pixel-level FID. The per-class FID and Virchow2 nearest-neighbor distances in Table~\ref{tab:progan-diagnostics} are therefore reported as descriptive quality indicators rather than as the cause of the null result; the nearest-neighbor distances in particular lack a real-to-real baseline and should not be read as direct evidence of off-manifold generation. The small focal-loss shift is consistent with the interpretation that difficulty reweighting is not very helpful when most cancer types are already separable in the frozen embedding space.
