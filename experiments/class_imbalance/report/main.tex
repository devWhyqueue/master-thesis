...
The patch benchmark compresses into a narrow performance band once patches are represented with frozen Virchow2 embeddings (Table~\ref{tab:patch-results}). The Scholz-style CE plus soft-MCC objective with balanced sampling ranks highest by macro F1 ($\num{0.782} \pm \num{0.007}$) and balanced accuracy ($\num{0.782} \pm \num{0.008}$). The closely related CE plus soft-F1 variant reaches nearly the same macro F1 ($\num{0.782} \pm \num{0.004}$) and balanced accuracy ($\num{0.782} \pm \num{0.004}$). Relative to plain CE ($\num{0.776} \pm \num{0.002}$ macro F1, $\num{0.770} \pm \num{0.003}$ balanced accuracy), the hybrid objectives provide small but consistent improvements over CE in the frozen-embedding patch regime (Table~\ref{tab:paired-deltas}): paired seed-wise macro-F1 deltas are positive for both Scholz-style variants in all three seeds, with mean gains of about $\num{+0.006}$--$\num{+0.007}$ points. The same methods do not improve probability quality on the same scale (Table~\ref{tab:calibration}): CE plus soft F1 raises NLL to $\num{2.20} \pm \num{0.09}$ and CE plus soft MCC to $\num{1.95} \pm \num{0.09}$, compared with $\num{1.81} \pm \num{0.09}$ for plain CE, while expected calibration error remains near $\num{0.14}$ for both hybrid objectives and the baseline.

The simpler patch interventions separate more clearly in this embedding regime. Class-weighted CE and balanced sampling remain competitive with CE, with macro F1 values of $\num{0.776} \pm \num{0.012}$ and $\num{0.775} \pm \num{0.008}$, respectively. ProGAN augmentation does not outperform the CE baseline: it reaches macro F1 $\num{0.774} \pm \num{0.008}$ and balanced accuracy $\num{0.767} \pm \num{0.011}$, which is within the spread of the non-generative methods rather than above them. This negative result is not explained by a lack of synthetic volume alone. For seed~0, ProGAN adds \num{348120} generated training patches across 31 classes below the head-class patch count (Table~\ref{tab:progan-diagnostics}) on top of the same real controlled training set as plain CE. Quality diagnostics show high Inception FID (median \num{178.5}) and non-trivial Virchow2 nearest-neighbor distances (median \num{47.9} in $L_2$ on \num{2560}-dimensional embeddings), indicating that the generated patches are neither near-exact copies of real training patches nor close to the real feature manifold used at classification time (Figure~\ref{fig:progan-examples}). Focal loss is the weakest patch method ($\num{0.766} \pm \num{0.003}$ macro F1, $\num{0.760} \pm \num{0.002}$ balanced accuracy), indicating that down-weighting easy examples is not helpful once the representation already separates most cancer types well. Accuracy remains high for all methods ($\approx \num{0.83}$), so the ranking is driven mainly by class-balanced metrics rather than by overall prevalence-sensitive performance.
...
RankMix shows the clearest WSI-bag benefit, especially in balanced accuracy, while macro-F1 gains remain modest (Table~\ref{tab:wsi-results}). Relative to plain MIL CE, balanced accuracy improves from $\num{0.836} \pm \num{0.020}$ to $\num{0.863} \pm \num{0.008}$, whereas macro F1 moves from $\num{0.840} \pm \num{0.020}$ to $\num{0.848} \pm \num{0.017}$ with overlapping standard deviations across seeds. Paired seed-wise comparisons (Table~\ref{tab:paired-deltas}) show positive RankMix deltas in all three seeds for both metrics, with the largest balanced-accuracy shift in seed~0 ($\num{+0.045}$). RankMix also improves probability calibration (Table~\ref{tab:calibration}), lowering NLL from $\num{1.03} \pm \num{0.05}$ to $\num{0.41} \pm \num{0.04}$, Brier score from $\num{0.195} \pm \num{0.009}$ to $\num{0.156} \pm \num{0.015}$, and ECE from $\num{0.089} \pm \num{0.003}$ to $\num{0.022} \pm \num{0.003}$. The larger balanced-accuracy shift is consistent with RankMix directly augmenting representative instance subsequences in the MIL regime.
