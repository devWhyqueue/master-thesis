...
Divide-and-conquer patch classification decomposes a long-tailed multiclass problem into three binary subproblems, trains separate experts on cluster-sampled subsets, and fuses their representations for the final decision \citep{nouyedb2025patternRecognition}. Nouyed et al.\ originally partition HiPath patches into tumor versus benign-in-cancerous-slide versus benign-from-benign-slide sets and train experts on \emph{A vs B}, \emph{A vs C}, and \emph{A vs (B+C)} with k-means cluster sampling and z-score stratification so the majority side matches the minority count while preserving texture diversity. This benchmark preserves the divide-and-conquer structure and sampling strategy but redefines the partitions using class difficulty and support tiers because TCGA-UT patches come from tumor regions with cancer-type labels only: set~A is the five hardest classes by CE-baseline test error, set~B is the eight lowest-support tail classes, and set~C is the eight highest-support head classes.
...
\begin{table}[ht]
  ...
  \caption{Patch-level test results over three seeds. Methods share the same controlled manifest, frozen Virchow2 encoder, shared MLP trunk, and evaluation code.}
  \label{tab:patch-results}
\end{table}

The patch benchmark compresses into a narrow performance band once patches are represented with frozen Virchow2 embeddings (Table~\ref{tab:patch-results}). CFAL ranks highest on both macro F1 ($\num{0.807}\pm\num{0.007}$) and balanced accuracy ($\num{0.801}\pm\num{0.003}$), with paired gains over plain CE of about $\num{0.031}$ on each headline metric in all three seeds (Table~\ref{tab:paired-deltas}). The Scholz-style hybrid objectives remain second: they improve macro F1 by only a few thousandths over CE, whereas CFAL provides a substantially larger shift without balanced oversampling. Divide-and-conquer (D\&C) occupies the representation-and-architecture slot on patches but falls below plain CE on both metrics ($\num{0.752}\pm\num{0.010}$ macro F1; paired deltas $\num{-0.024}\pm\num{0.010}$), indicating that decomposing the problem into difficulty-tier binary experts does not recover the gains seen with prototype-based affinity training under a frozen Virchow2 encoder.
