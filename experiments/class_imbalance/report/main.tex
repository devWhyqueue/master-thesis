...
\begin{table}[ht]
  ...
  \begin{tabularx}{\linewidth}{@{}lX@{}}
    ...
    Leakage-control status & Slide-disjoint; not patient-disjoint under TCGA participant-barcode audit (Section~\ref{sec:shared-splits})\\
...
For each seed, slides are split into \num{6116} training, \num{1310} validation, and \num{1310} test slides. The same slide-level assignments are reused by both benchmarks so that no patches from the same slide can appear in different splits. This slide-disjoint partition is the implemented leakage-control mechanism: the patch benchmark and the WSI-bag benchmark see different input units, but they inherit the same train/validation/test slide partition.

Slide-disjoint splitting is necessary for this benchmark, but it is not equivalent to patient- or case-disjoint splitting. A leakage audit on the generated split files derived case identifiers from the TCGA participant barcode prefix and found cases with multiple diagnostic slides assigned to different splits. The number of cross-split cases was 415 for seed~0, 419 for seed~1, and 435 for seed~2; for example, participant \texttt{TCGA-OR-A5J1} has diagnostic slides in both training and validation in seed~0. Therefore the reported results should be interpreted as slide-disjoint estimates rather than patient-disjoint estimates. A patient-disjoint benchmark would require regenerating the manifests and rerunning all training, tuning, and test evaluation with split assignment performed at the TCGA participant level.
...
The simpler patch interventions separate mainly by what they fail to improve after validation selection. Class-weighted CE and balanced sampling remain near CE on mean macro F1, and ProGAN augmentation stays within the non-generative spread despite adding a large synthetic tail-class volume (Table~\ref{tab:progan-diagnostics}). The accompanying FID and Virchow2 nearest-neighbor diagnostics indicate that this is not merely a quantity problem: many generated patches are visually degenerate or near-monochrome and are distant from the real feature manifold used by the classifier (Figure~\ref{fig:progan-examples}). Focal loss does not improve over CE, consistent with the interpretation that difficulty reweighting is not helpful when most cancer types are already separable in the frozen embedding space.
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.23\linewidth}p{0.68\linewidth}}
    ...
    \emph{Slide-disjoint split} & Split rule preventing any slide's patches or features from crossing train, validation, and test; this does not by itself prevent different slides from the same patient from crossing splits.\\
    \bottomrule
  \end{tabular}
