...
RankMix shows the clearest WSI-bag benefit, especially in balanced accuracy, while macro-F1 gains remain modest (Table~\ref{tab:wsi-results}). Paired seed-wise comparisons show positive RankMix deltas in all three seeds for both headline metrics (Table~\ref{tab:paired-deltas}), and calibration improves more clearly than discrimination (Table~\ref{tab:calibration}). This is consistent with RankMix acting on the MIL-specific bottleneck: it augments representative instance subsequences rather than only reweighting already observed bags.

The remaining WSI methods cluster closely. Balanced-sampling MIL and SC-MIL produce similar balanced accuracy ($\num{0.837}$), but both trail RankMix in macro F1. SC-MIL performs well on validation but does not exceed plain MIL CE on the test macro-F1 average. Weighted MIL CE and focal MIL do not improve over plain MIL CE in this configuration, with focal MIL giving the lowest WSI macro F1. Activation diagnostics confirm that the advanced WSI objectives were active: RankMix generated about \num{1100000} mixed examples per seed, and SC-MIL constructed about \num{1060000} positive pairs per seed.
...
The tuned robustness check partly narrows the negative result but does not overturn it. On patches, validation tuning gives small test gains for weighted CE and balanced sampling, yet the two Scholz-style hybrid objectives remain the strongest patch methods; soft MCC selects its original metric-loss weight, and soft F1 improves only marginally with a larger weight. On WSI bags, RankMix is the only tuned method that clearly exceeds plain MIL CE on held-out macro F1, with $\alpha=\num{2.0}$ improving both macro F1 and balanced accuracy. Weighted CE, balanced sampling, and SC-MIL improve relative to their fixed settings but still do not surpass MIL CE on test macro F1, and focal loss remains below baseline. The fixed-feature, strong-baseline bottleneck is therefore still supported, although the tuned results show that some simple mitigation variants were mildly under-tuned.
...
Several limitations remain. TCGA-UT patch labels come from pathologist-selected tumor regions; they are useful patch-level class labels but not dense annotations. The two benchmarks share slide-disjoint splits but differ in prediction unit and per-slide evidence budget, so their result tables remain non-transferable as discussed above. The experiments also use fixed, shared training budgets and method defaults rather than an exhaustive validation search over focal $\gamma$, class-weight scaling, sampler strength, hybrid-loss weighting, MIL hidden dimensions, RankMix mixing parameters, or ProGAN architecture and training schedule. Method-specific tuning could improve some metrics substantially; this study therefore supports a conservative claim about the tested fixed-protocol setting rather than a broad negative claim about imbalance mitigation.
