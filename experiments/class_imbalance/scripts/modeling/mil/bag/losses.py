...
from scripts.modeling.mil.bag.dataset import AttentionMil, DualExpertMil
...
def _mde_mil_loss(
    ...
    loss_cls = nn.functional.cross_entropy(
        logits_u, targets_u
    ) + nn.functional.cross_entropy(logits_b, targets_b)
    lambda_con = float(config["mde_mil_consistency_weight"])
    loss_con = nn.functional.mse_loss(
        logits_u, logits_u_cross
    ) + nn.functional.mse_loss(logits_b, logits_b_cross)
