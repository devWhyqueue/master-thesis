"""Command-line entry point for the prior-vs-support cause experiment."""
...
from cause.analyze import run_analyze
from cause.fit import run_fit_shard, shard_count
...
def _parser() -> argparse.ArgumentParser:
    """Create the cause command-line argument parser."""
    parser = argparse.ArgumentParser(
        description="Prior-vs-Support Cause Experiment CLI"
    )
...
def cmd_fit(args: argparse.Namespace) -> None:
    """Fit every pending P/S arm of one (split, draw) shard, then its LP/Lr derivatives."""
    run_fit_shard(load_config(args.config), args.shard_index)


def cmd_analyze(args: argparse.Namespace) -> None:
    """Pool reused and new arm accuracy, write analysis/diagnostics, and the figure."""
