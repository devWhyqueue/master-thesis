...
from imbalance_benchmark.common import REPO_ROOT, resolve_partitioned_setting
...
def _directives(job: SlurmJob, root: str, config: dict[str, Any]) -> list[str]:
    log_dir = (
        f"{shlex.quote(root)}/experiments/01_benchmark/02_benchmark_patch/outputs/logs"
    )
    lines = [
        "#!/bin/bash",
...
def _execution_lines(
    ...
    data_binds = _data_binds(config, job.name)
    binds = f'-B "{root}:{root}:ro" -B "{output_dir}:{output_dir}:rw" {data_binds}'
    if dataset_root:
...
def _cluster_paths(config: dict[str, Any]) -> tuple[str, str, str, str]:
    slurm = config.get("slurm", {})
    root = str(slurm.get("project_root", REPO_ROOT))
    benchmark = posixpath.join(root, "experiments/01_benchmark/02_benchmark_patch")
