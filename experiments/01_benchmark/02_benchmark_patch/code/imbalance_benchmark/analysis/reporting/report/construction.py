...
def _independent_counts(entry: dict[str, Any]) -> tuple[int, int]:
    ...
    """
    frame = pd.read_csv(entry["path"], usecols=cast(Any, ["case_id", "slide_id"]))
    ...
    rendered = pd.DataFrame(
        {
            ...
            "Split": frame["split"],
            "Condition": frame["condition"].map(cast(Any, CONDITION)),
            "Assignment": frame["assignment"].map(cast(Any, ASSIGNMENT)),
            "Units": frame["units"],
            "Slides": frame["slides"],
