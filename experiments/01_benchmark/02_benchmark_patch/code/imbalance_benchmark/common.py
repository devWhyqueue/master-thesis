...
EXPERIMENT_ROOT = REPO_ROOT / "experiments" / "01_benchmark" / "02_benchmark_patch"
DEFAULT_CONFIG_PATH = EXPERIMENT_ROOT / "configs" / "default.yaml"
N_PATIENT_SPLITS = 3
RUN_RECORD_NAME = "run.json"
EVAL_ARRAYS_NAME = "eval_arrays.npz"
ARRAY_FIELDS = ("labels", "preds", "probabilities", "logits", "raw_probabilities")
...
def output_root(config: dict[str, Any]) -> Path:
    ...
    configured = Path(
        config.get("paths", {}).get(
            "outputs", "experiments/01_benchmark/02_benchmark_patch/outputs"
        )
    )
