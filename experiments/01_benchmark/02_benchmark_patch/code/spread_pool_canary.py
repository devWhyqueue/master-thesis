...
CONFIGS = {
    "bracs": "experiments/01_benchmark/02_benchmark_patch/configs/bracs_patch.yaml",
    "camelyon16": "experiments/01_benchmark/02_benchmark_patch/configs/camelyon16_patch.yaml",
    "panda": "experiments/01_benchmark/02_benchmark_patch/configs/panda_patch.yaml",
    "tcga_ut": "experiments/01_benchmark/02_benchmark_patch/configs/tcga_ut_patch.yaml",
}
...
def _class_row(train_df, cls, counts, seed, floor):
    ...
    try:
        ...
        row["nested"] = bool(set(wide["case_id"]).issubset(set(spread["case_id"])))
...
def main() -> None:
    """Execute canary sampling on configured datasets and output JSON results."""
    ...
    for dataset in only:
        ...
        for index in range(3):
            ...
            logger.info("done %s split %d", dataset, index)
        out.append({"dataset": dataset, "splits": splits})
    logger.info(json.dumps({"canary": out}, indent=1))
