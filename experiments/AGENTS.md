...
| `03_class_imbalance/` | `00_damage/`: 25_damage_tcga_ut, 26_damage_bracs. `01_cause/`: `tcga_ut/`: 27_prior_vs_support, 30_tail_assignment, 31_worst_assignment, 32_worst_assignment_rate. `bracs/`: 28_prior_vs_support, 29_tail_assignment. `cross/`: 33_prior_margin |
...
| 03_class_imbalance | 25–33 | Why does class imbalance hurt accuracy, and what causes it? | TCGA-UT: slow, accelerating BA/NLL/ECE damage with prevalence ratio; BRACS: early, steady, ~6–7× larger damage; temperature scaling removes most of the calibration cost on both (exp-25–26). TCGA-UT cause (exp-27): skewed prior and thin tail support interact — at ρ 100 prior alone 1.20, support alone 0.53, interaction 1.18 of 2.91 pp; prior carries the damage up to ρ 20, support + interaction the acceleration beyond ρ 10; training-time rebalancing beats post-hoc logit adjustment. BRACS cause (exp-28): prior (4.47) and support (3.29) both damage, additive in point estimate (I −0.11, unresolved); prior larger but thin support alone exceeds TCGA-UT's full damage. BRACS tail class (exp-29): damage at ρ 100 ranges 5.7–10 pp by which class is rare, ordered by balanced recall (headroom), hardest classes cheapest; six of seven classes collapse to ≤ 4% recall in the tail. TCGA-UT tail assignment (exp-30): easy- vs hard-tail order differ only 0.82 pp; both sorted orders beat random orders (by 0.51/1.33); no tail collapse, recall follows allocation relative to confusion partner (COAD/READ, HNSC/LUSC); per-class own-allocation model does not generalize. A directed pair-separation score fails its own pre-registered gate on TCGA-UT (exp-31) and a rate-weighted fix passes it, confirming direction and rank separation both cost accuracy (exp-32). Why the exp-27/28 prior-channel damage itself is 3.27 pp larger on BRACS is not settled: injecting each dataset's own fitted prior shift into the balanced model's stored decisions overshoots the real fitted damage by 3–9× on both datasets (exp-33, gate fails, no mechanism claim), though BRACS's balanced-arm accuracy and margin sit outside the range of every 7-class subset carved from TCGA-UT — a narrower class count alone does not explain it |
