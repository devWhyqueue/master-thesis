...
Methods are compared within a regime, never by treating patch and WSI-bag predictions as exchangeable. Balanced accuracy (equivalently macro recall in single-label multiclass classification) is the primary endpoint because it gives every class equal weight. Macro F1, classwise precision/recall/F1, and class-group head/body/tail recall are secondary discrimination outcomes. These quantities do not by themselves establish whether reported probabilities are trustworthy. The framework therefore also reports NLL and multiclass Brier score, reliability diagrams, and ECE as a bin-dependent secondary summary \citep{mosquera2024classImbalance}.
...
\section{Summary}
\label{sec:summary}
This report defines the benchmark's no-mitigation references (CE and MIL CE) and its retained interventions: balanced sampling, class-weighted CE, focal loss, train-time and post-hoc logit adjustment, soft-F1, soft-MCC, CFAL, OKO, RankMix-inspired feature mixing, cRT, SC-MIL, and MDE-inspired dual-expert MIL. Balanced Softmax is recorded as related prior-correction literature rather than an additional tested method. The definitions make the tested adaptations and ablations explicit and establish balanced accuracy as the primary endpoint, with discrimination and calibration evaluated jointly in the companion experimental protocol.
