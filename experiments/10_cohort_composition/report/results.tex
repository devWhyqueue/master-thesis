...
Both class groups show a concentration damage above the threshold, but it is twice as large in the multi-site classes, at 11.50 points, as in the seventeen other classes, at 5.86 points (Table~\ref{tab:contrasts}). The coverage ratios of the two groups differ much less, 0.66 against 0.60. The clustered cohort loses most in the multi-site classes, at 57.61 percent, and the other classes include the five types that receive almost no concentration contrast. The selection gain follows the size of its coverage change: 1.29 points at a ratio of 0.38 for multi-site classes and 0.86 points at 0.18 for the other classes, whose interval includes zero.

Within classes, the damage falls on test patients whom the clustered cohort covers worse (Table~\ref{tab:tertiles}). In the tertile with the smallest coverage improvement, clustered training is 2.28 points more accurate than random training. In the middle tertile, the damage is 7.65 points, and in the tertile with the largest coverage improvement, 20.44 points, where recall under clustered training drops to 46.53 percent. The gradient appears in both class groups, most steeply in the multi-site classes.
...
Coverage-improvement tertile & Clustered & Random & Damage & Multi-site & Other \\
\midrule
...
The asymmetry between the two contrasts matters for patient shortage. Random sampling at twenty patients already attains most of the coverage that coverage-maximizing selection can reach, and it attains most of the accuracy too. Cohort composition therefore protects mainly against a poorly covering cohort, such as one drawn from a single clinic's typical cases, and offers at most a small gain when a random cohort of twenty patients is available. Under the concentration-damage reading, the value of coverage-guided selection remains open where random cohorts cover a class less well.

Before selection is tested at five patients, two properties of a cohort need to be separated: how well its patients cover the class, and how similar they are to one another. Effective support already counts the correlation $\bar\rho_c$ of patches within a patient \citep{queisler2026multidirectionalRedundancy}. It can also count similarity between patients. If $\omega$ is the mean correlation between the deviations of two cohort patients from their class mean, effective support becomes $Gm/[1+(m-1)\bar\rho_c+m(G-1)\bar\rho_c\,\omega]$. The added term is zero on average for random cohorts and positive for a clustered cohort. The next experiment therefore asks two questions of the stored classifiers of the breadth--depth grid, whose cohorts are random. Do similarity-adjusted effective support and coverage distance together account for the residual benefit of additional patients? And does the relation fitted to these random cohorts reproduce both contrasts of this report? If it does, the relation yields a forecast, fixed before any classifier is trained, for coverage-selected against random cohorts of five patients with 32 patches each, the allocation in which patient shortage was first observed.
