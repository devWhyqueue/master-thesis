...
\bottomrule
\end{tabular}
\caption{Class-level leakage stays above the permitted 0.5 for every cancer type except testicular germ cell tumours. Coverage distances and leakage are averaged over the three splits and five draws; $\kappa_c$ is computed from the class's own coverage distances. The focal-site shares give the percentage of added patients that come from the tissue source site of the focal anchor, for the concentrated and the random addition rule.}
\label{tab:class-check}
\end{table}

Concentration restricts institutions only mildly. On average, 27 percent of the concentrated additions share the focal anchor's tissue source site, against 17 percent of the random additions. Two classes with few contributing sites, adrenocortical carcinoma and pheochromocytoma and paraganglioma, reach about 70 percent under both rules, so the difference between the rules there is nil. The overlap with site coverage that Section~\ref{sec:limitations} anticipates is thus small compared with the failure of the coverage manipulation.

\subsection{Why concentration does not withhold coverage}
\label{sec:diagnosis}
The design rests on one premise: a validation patient whose nearest anchor is not the focal anchor should gain nothing from the concentrated additions. A diagnostic on the same embeddings, draws, and allocations tests this premise directly. It reproduces the coverage distances of Table~\ref{tab:check} exactly and splits each allocation's improvement $r_{\mathrm D}-r$ into the contributions of validation patients in the focal cell and in the other cells, each expressed as a share of the random allocation's total improvement $r_{\mathrm D}-r_{\mathrm R}$ (Table~\ref{tab:decomposition}).
...
\bottomrule
\end{tabular}
\caption{Most of the concentrated allocation's coverage improvement comes from validation patients outside the focal cell, which the design assumed it would not reach. The focal cell holds the validation patients whose nearest anchor is the focal anchor. The middle columns give each cell's contribution to an allocation's coverage improvement as a percentage of the random allocation's total improvement; the concentrated columns sum to $100\kappa$. The last column gives the percentage of other-cell validation patients whose nearest training patient is closer in the concentrated than in the deep allocation.}
\label{tab:decomposition}
\end{table}

\noindent The focal cell behaves as intended but is small. It holds 17 to 23 percent of the validation patients, close to the one fifth expected from five anchors, and concentrated additions improve coverage there only slightly more than random additions do, contributing 21 to 29 percent of the random allocation's improvement against 18 to 25 percent. The premise fails in the other cells. For 55 to 57 percent of the validation patients outside the focal cell, one of the fifteen neighbours of the focal anchor lies closer than their own nearest anchor, and these patients contribute 43 to 48 percent of the random allocation's improvement, about twice the contribution of the focal cell. Leakage thus arises mainly where the design expected none.

The reason is the scale of the anchor cells. Five randomly drawn anchors leave validation patients at a mean distance of 0.39 from their nearest anchor, so a cell is not a tight neighbourhood but a large share of the class. The fifteen patients nearest to one anchor spread over a region that lies closer to many validation patients than their own anchor does. The same diagnostic shows how weakly the coverage distance separates cohorts by composition: twenty patients consisting of the focal anchor and its nineteen nearest neighbours cover validation patients at 0.346 to 0.356, better than the five random anchors at 0.385. Under the patient-mean embedding, adding almost any patients shortens the nearest-patient distance for many held-out patients.

\subsection{Alternative allocation rules}
\label{sec:alternatives}
Because each manipulation check costs no classifier fits, the diagnostic also evaluated alternative allocation rules against the same criterion before any redesign is fixed. Two coverage-maximizing rules select from the training pool without using validation patients: \emph{farthest-point selection} repeatedly adds the pool patient farthest from the current set, and \emph{facility-location selection} repeatedly adds the pool patient that most lowers the mean distance from all pool patients to their nearest selected patient. For an upward contrast, the random allocation is the low arm, and the criterion requires it to attain at most half of the high arm's improvement. A \emph{fully concentrated} variant replaces the random anchors by the focal anchor and its four nearest neighbours, so that the deep allocation is itself concentrated and must be fitted anew. Table~\ref{tab:alternatives} lists the leakage of every variant.
...
\midrule
Anchors + neighbours of all anchors & Anchors + random & 0.84 & 0.85 & 0.88 & 0 of 90 \\
Anchors + neighbours of focal anchor & Anchors + random & 0.69 & 0.67 & 0.73 & 6 of 90 \\
Anchors + random & Anchors + farthest-point & 1.39 & 1.35 & 1.37 & 0 of 90 \\
Anchors + random & Anchors + facility-location & 0.80 & 0.80 & 0.80 & 1 of 90 \\
Anchors + neighbours of focal anchor & Anchors + facility-location & 0.55 & 0.53 & 0.59 & 36 of 90 \\
Focal anchor + 19 neighbours & Focal anchor + 4 neighbours + random & 0.64 & 0.61 & 0.66 & 13 of 90 \\
Focal anchor + 19 neighbours & Focal anchor + 4 neighbours + facility-location & 0.56 & 0.53 & 0.57 & 32 of 90 \\
\bottomrule
\end{tabularx}
...
Whether the residual breadth benefit of TCGA-UT is a patient-coverage benefit remains open after a second stopped manipulation check. Concentrating all additions around one anchor lowers leakage from 0.84--0.88 to 0.67--0.73, but no classifier was trained, so neither the coverage gain nor the within-concentration residual constrains the accounts of Table~\ref{tab:accounts}.

The two checks together yield a result about the design space. The diagnostic refutes the assumption on which the concentrated design rests: anchor cells do not localize coverage, and validation patients outside the focal cell supply about twice as much of the concentrated allocation's improvement as those inside it. Across seven nested variants, including upward contrasts with coverage-maximizing rules, the best leakage is 0.53. Under the patient-mean embedding, any nested design that must add little coverage while quadrupling patients runs against the fact that quadrupling patients shortens the nearest-patient distance for most held-out patients, whichever patients are added. A clean within-concentration residual, which requires a step that raises patient count at nearly constant coverage, is therefore out of reach for this coverage measure.

The measure is not insensitive to composition, however. Its dynamic range between cohorts of equal size is large once no five-patient allocation has to be nested inside both arms: in the diagnostic, twenty patients forming one tight neighbourhood cover validation patients at 0.346 to 0.356, and twenty patients selected by facility location starting from the same focal anchor cover them at 0.268 to 0.271. This difference is almost as large as the coverage change from five to twenty random patients. The question whether dissimilar patients help can thus still be asked directly, at a fixed patient count, even though the question whether similar patients help beyond effective support cannot be asked with nested allocations.

\subsection{Follow-up: coverage, random sampling, and learning}
\label{sec:follow-up}
A follow-up experiment would compare three allocations of twenty patients contributing eight patches each: the focal anchor and its nineteen nearest neighbours (\emph{clustered}), nineteen randomly drawn additions (\emph{random}), or nineteen additions selected by facility location on the training pool (\emph{dispersed}). All three share the focal anchor, patient count, patch budget, and patch-sampling rule. The random allocation distinguishes damage from deliberate concentration from improvement over ordinary sampling. Three splits and five draws require 45 fits, with the same logistic-regression readout and validation tuning as this experiment.

The clustered--dispersed coverage contrast is already substantial: it reaches 0.90, 0.95, and 0.84 times the coverage change from five to twenty random patients. The follow-up would retain a minimum ratio of 0.5 in every split as its manipulation check, while reporting class-level contrasts, especially for the seventeen smaller cancer types. These diagnostics establish feasibility, not an expected accuracy gain. Before fitting, the coverage measure and analysis would be fixed; an exploratory analysis of stored random allocations would test whether coverage predicts accuracy among draws with identical patient counts and patch depths, and adds predictive value beyond effective support and patient count.

The primary contrasts would be dispersed minus random accuracy and random minus clustered accuracy, using patient-macro balanced accuracy, paired test-patient intervals, and the one-percentage-point threshold. A gain over random sampling would support coverage-guided selection; a gain only over clustering would establish concentration damage without demonstrating improved ordinary sampling. An interval within $[-1,1]$ would indicate practical equivalence for the corresponding contrast at this allocation size, whereas an interval crossing a threshold would remain inconclusive. Training-draw sensitivity and the association between each test patient's coverage improvement and recall change would be reported alongside, separately for the thirteen multi-site classes and the seventeen remaining classes.

Equal calculated effective support does not ensure equal dependence between selected patients, and changes in site composition or within-class frequencies can also affect accuracy. The contrasts therefore test selection strategies without uniquely identifying coverage as the cause of the original breadth benefit. If coverage predicts ordinary-cohort performance and selection improves it, a subsequent comparison of coverage-selected and random five-patient cohorts at the same patch budget would test mitigation under shortage. A null result would weaken patient-mean coverage at the tested size, rather than exclude coverage generally; diagnostically relevant variation in patients' patch distributions would remain a candidate.
