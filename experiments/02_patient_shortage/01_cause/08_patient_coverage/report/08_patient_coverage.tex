...
\maketitle
\begin{abstract}
\noindent At a fixed patch budget, TCGA-UT patch classifiers trained on more patients are more accurate than redundancy-adjusted effective support predicts, and coverage of tissue source sites explains little of this residual. This experiment tests whether additional patients help because they cover more of the variation between patients of a cancer type. Patient similarity is measured without labels, as the cosine distance between patient-mean Virchow2 embeddings, so the design applies to all 30 cancer types, including the smaller ones in which most of the residual arises. Three nested training allocations share a budget of 160 patches per class: five randomly drawn anchor patients with 32 patches each, twenty patients formed by the anchors and their nearest neighbours with eight patches each, and twenty patients formed by the anchors and randomly drawn patients with eight patches each. The first step quadruples patients while adding little coverage; the second adds coverage while patients, patches, and effective support stay fixed. The \emph{coverage gain} compares the two broad allocations, and the \emph{within-neighbourhood residual} measures what the first step adds beyond effective support. A coverage gain above one percentage point together with a within-neighbourhood residual below one point per doubling of patients would identify patient coverage as the missing source of information; the reverse pattern would locate it in properties of additional patients that their mean embedding does not express. A census run before any classifier is trained requires the neighbour allocation to attain at most half of the coverage improvement of the random allocation. It attains 84 to 88 percent of it in the three splits, and no cancer type meets the requirement, so the experiment stops at the census and both contrasts remain unmeasured. The cause is neither the size of the patient pools nor the selection rule, which picks patients twice as close to their anchors as random additions are, but the geometry of the coverage measure: patients clustered around five fixed anchors shorten the distance from held-out patients to their nearest training patient almost as much as scattered patients do, because the two rules differ only for held-out patients far from every anchor. Withholding coverage at a fixed patient count therefore requires a selection rule built around the coverage measure itself, and coverage distance responds far more to how many patients a cohort holds than to which patients they are.
\end{abstract}
...
Coverage of tissue source sites, the institutions that contributed TCGA tissue, accounts for little of this residual \citep{queisler2026siteCoverage}. For the thirteen cancer types with enough patients per site, doubling the number of sites at a fixed patient count adds 1.06 points (0.74 to 1.37), and most of this gain falls on test patients from the added sites, which is the pattern of site matching rather than of robustness to new institutions. Quadrupling patients within fixed sites adds 3.31 points per doubling beyond effective support (2.71 to 3.84), but most of this value reflects a weak deep site allocation rather than a benefit specific to patients from represented sites. Two gaps remain. First, additional patients carry information that neither effective support nor site count describes. Second, these thirteen larger cancer types carry only 1.21 points of residual per doubling, so most of the residual arises in the seventeen smaller types, which lack enough patients per site for a site intervention.
...
The second rule nests the patients of the three allocations. For each split, class, and draw $d\in\{1,\ldots,5\}$, the five anchors are drawn with the five-patient sampling rule of \citet{queisler2026effectiveSupport}, using that study's split-0 class order for every split. The anchors therefore coincide with its stored five-patient draw only in split 0, so the stored predictions of the grid cell of five patients contributing 32 patches each serve as the deep allocation only there; in the other two splits the deep allocation is fitted afresh. The anchors are placed in a random order. In each of three rounds, every anchor in turn selects the patient of $\mathcal E_{rc}$ closest to it under Equation~\eqref{eq:distance} among those not yet selected, with ties broken by patient identifier. The broad random allocation adds fifteen patients drawn without replacement from the pool without the anchors. Writing $\mathcal P^{\mathrm D}$, $\mathcal P^{\mathrm N}$, and $\mathcal P^{\mathrm R}$ for the patient sets of the three allocations,
\begin{equation}
 ...
 \label{eq:nesting}
\end{equation}
\noindent Because the anchors are a random draw and the remaining patients are drawn at random from the rest of the pool, the broad random allocation is distributed exactly like the twenty-patient draw of the breadth--depth grid; it differs only in being paired with the deep draw. Patches follow the nested round-robin order over each patient's slides, so the eight patches of an anchor in either broad allocation are the first eight of its 32 deep patches. Classes are sampled independently. Both broad allocations are fitted in all three splits and the deep allocation in splits 1 and 2, each for five draws.
...
\label{sec:contrasts}
Accuracy is patient-macro balanced accuracy over all 30 classes: patch recall is averaged within each test patient and true class, then equally across patients within each class, and finally across classes. An allocation score $A$, in percent, averages the three splits and five draws equally, giving $A_{\mathrm D}$, $A_{\mathrm N}$, and $A_{\mathrm R}$ for the deep, neighbour, and random allocations. In split 0, the deep allocation reproduces the stored grid cell of five patients contributing 32 patches each by construction. Across all splits, $A_{\mathrm D}$ is expected to lie close to that cell, and $A_{\mathrm R}$ is expected to lie close to the grid cell of twenty patients contributing eight patches each.
...
\begin{equation}
 ...
 \label{eq:neff}
\end{equation}
\noindent which gives 10.06 per class for the deep and 36.58 for both broad allocations. The part of the first step that effective support explains is $\hat\beta\log\bigl(\Neff(20,8)/\Neff(5,32)\bigr)$, where $\hat\beta$ is the effective-support slope of the regression $A=\alpha+\beta\log\Neff+\gamma\log G$ fitted to the stored predictions of the nine-cell breadth--depth grid. Because all classes enter, this regression is the full-feature regression of \citet{queisler2026multidirectionalRedundancy}, and its reference residual $b_{\mathrm{ref}}=\hat\gamma\log 2$ must reproduce 2.62 points per doubling. Dividing the remainder of the first step by its two doublings of patients gives the \emph{within-neighbourhood residual breadth benefit}
\begin{equation}
...
The two broad allocations differ in more than spread. Nearest neighbours are drawn from dense regions around the anchors and may share tissue source sites or preparation batches with them, because site-specific signatures are visible in deep features of TCGA slides \citep{howard2021impact}. The neighbour step then partly repeats the within-site step of \citet{queisler2026siteCoverage}, and the random step partly adds sites. The site gain of 1.06 points measured there offers a reference scale for how much of a coverage gain site coverage alone could explain, and the reported site shares quantify the overlap, but the design does not remove it.

The manipulation is weakest where it matters most. In a pool of 30 patients, the fifteen nearest neighbours of five anchors make up half the pool and may be hardly closer to the anchors than randomly drawn patients. The smaller cancer types, which carry most of the residual breadth benefit, therefore contribute the least contrast in coverage. The class-level leakage $\kappa_c$ identifies these classes, and the subgroup analysis separates them from the multi-site classes. Finally, the within-neighbourhood residual inherits the assumptions of the effective-support regression, whose slope is estimated on randomly drawn allocations and transferred to similarity-restricted ones, and all results are conditional on frozen Virchow2 features, multinomial logistic regression, the three overlapping splits, and the tested allocation sizes.
