...
\caption{Balanced accuracy degrades slowly and accelerates beyond $\rho=10$. The line shows the change in patient-macro balanced accuracy relative to the balanced arm, with the ratio arms at their realized ratio and a shaded paired 95\% bootstrap band; the red diamond marks the native allocation at its realized ratio of 18.1.}
\label{fig:curve}
\end{figure}
...
For the prevalence factor, the curve replaces the benchmark's three-allocation readout. The benchmark measured 1.33 to 1.79 points of severe-imbalance damage on TCGA-UT \citep{queisler2026benchmarkResults}, less than the 2.91 points at $\rho=100$ here. The benchmark's imbalanced conditions also changed the patient pool and compared against a different reference allocation and budget, so the two magnitudes are not directly comparable; both place the damage of severe prevalence imbalance on TCGA-UT at a few points. What the curve adds is its shape: the benchmark could not show that the damage stays below one point up to $\rho\approx10$ and accelerates beyond it, nor that the native distribution, at a realized ratio of 18, costs just above the one-point threshold. That damage of 1.18 points is also small compared with the patient-count effects of this series: at 160 patches per class, going from five to twenty patients per class gains 9.41 points \citep{queisler2026centreError}. In a long-tailed TCGA-UT training set, the number of patients per class is therefore likely to matter more than the prevalence of the class, provided the rare classes still receive a few patches from each of their patients.
...
This study is the first of the studies that split the benchmark into its deprivations. The remaining ones vary patch count per class at fixed prevalence and patient count at fixed patch count, one factor at a time and with the same plain baseline. Together with this curve, they replace the benchmark's bundled conditions and show which of its factors carries the damage it measured.
