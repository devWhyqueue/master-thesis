...
Four points depart from the frozen plan. First, the engineering pilot selected the lower grid endpoint $\lambda=10^{-8}$ once, for UNI2-h's R100 arm on TCGA-UT, on a validation plateau ($0.7025388$ against $0.7025316$ at $10^{-7}$); the grid was not expanded, contrary to the protocol's expansion rule. Second, 20 of the 840 main selections landed on a grid endpoint: four UNI2-h fits on TCGA-UT (R10 and R100, at $10^{-8}$); two Virchow2 R100 fits on BRACS at $10^{-8}$; and fourteen UNI2-h fits on BRACS, eight at $10^{-8}$ (P10, P100, R100) and six at $10^{2}$ (B, S10, S100). The protocol prescribes adjacent-decade diagnostic refits for such cells; they were not run, and every estimate above uses the frozen grid. Third, the phase-two input audit mislabeled test-only patch counts as validation-plus-test counts; a signed amendment corrected the counts without changing the schedule, model, grid, or inference plan. Fourth, the per-split contrasts, the paired probability-quality contrasts, the per-class recall, and the validation diagnostics were computed after the locked analysis, by a separate read-only pass over the same accepted fit records; no test result informed any selection.
