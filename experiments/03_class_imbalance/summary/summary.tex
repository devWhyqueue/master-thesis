...
\nextgroupplot[title={TCGA-UT (30 classes, $T=19{,}200$)},ylabel={Training patches},xmin=1,xmax=30,
...
\nextgroupplot[title={BRACS (7 classes, $T=2{,}240$)},xmin=1,xmax=7,xtick={1,...,7},
...
The balanced conditions reach 72.1\% balanced accuracy on TCGA-UT and 39.6\% on BRACS. On TCGA-UT, published ROI-level linear evaluations of 32 classes score 0.60--0.68 for current encoders (Virchow2 0.62, UNI2-h 0.68; \citealp{mahmoodlab2025uniRepo}); the higher value here reflects 30 classes and aggregation over each patient's patches. For seven-class BRACS subtyping, the CNN baseline of \citet{brancati2022bracs} reaches 55.7\% ROI-level balanced accuracy, calculated as the mean of the seven class recalls in their Table~9. \citet{vaidya2025threads} report slide-level balanced accuracy of 34.8\% with mean-pooled Virchow features, 41.9\% with PRISM, and 48.1\% with THREADS, all using linear probes and five patient-disjoint splits of the 189-patient cohort. The 39.6\% here lies within the reported slide-level range, but this study computes patient-macro recall from patch predictions. Different training budgets and evaluation units make these results context for task difficulty, not matched performance benchmarks.

Imbalance damage differs strongly between the two tasks (Figure~\ref{fig:damage}, left). On TCGA-UT it is small and accelerating: 0.57 pp at $\rho=10$, the 1-pp threshold only at $\rho=20$, and 2.91 pp [2.48, 3.34] at $\rho=100$. On BRACS it starts immediately and grows steadily by about 1.15 pp per doubling of $\rho$: 1.15 pp already at $\rho=2$, 4.02 pp at $\rho=10$, and 7.65 pp [5.77, 9.78] at $\rho=100$, a fifth of the balanced level. Both native allocations lie on their curves (1.18 and 4.05 pp). The aggregate hides a large shift of recall from tail to head: at $\rho=100$, the TCGA-UT tail third loses 11.8 pp while the head gains 5.4 pp; on BRACS the two tail classes drop from 38.7\% to 7.3\% recall. The predicted probabilities suffer far more than accuracy (Figure~\ref{fig:damage}, right). The expected calibration error (ECE), the average gap between the model's confidence and its accuracy, rises from 5.5 to 17.8 pp on TCGA-UT and from 12.3 to 35.7 pp on BRACS at $\rho=100$: the model becomes overconfident, but uniformly so. A global confidence correction that divides all logits by one scalar fitted on validation data (temperature scaling; \citealp{guo2017calibration}) removes 92--96\% of the increase.

\begin{figure}[t]
\centering
\begin{tikzpicture}
\begin{axis}[name=curve,width=0.47\textwidth,height=5cm,xmode=log,log basis x=10,
...
\end{axis}
\begin{axis}[at={(curve.east)},anchor=west,xshift=1.2cm,width=0.48\textwidth,height=5cm,
...
\end{axis}
\end{tikzpicture}
\caption{Left: balanced-accuracy damage versus $\rho$ with 95\% bands; diamonds mark native class shares, dotted line the 1-pp threshold. Right: expected calibration error (ECE) of the raw predicted probabilities (solid) and after a single global confidence correction fitted on validation data (dashed).}
\label{fig:damage}
\end{figure}
...
When a tail class receives fewer training patches, two things change. Its errors count less often in the training loss, so common classes have more influence on the fitted decision boundary. This is the \emph{prior} effect: it concerns how strongly the available examples of each class shape the classifier. The model also sees fewer distinct tail patches, at $\rho=100$ only one or two per patient, so it has less evidence about how that class varies. This is the \emph{support} effect: giving those few patches more weight cannot supply the missing examples. The conditions in Table~\ref{tab:conditions} separate these effects. P keeps all patches from the balanced condition but gives head classes more weight, isolating the prior effect. S keeps the sparse patches from the imbalanced condition but weights classes equally, isolating the support effect. The interaction $I=\Delta_\mathrm{r}-\Delta_\mathrm{P}-\Delta_\mathrm{S}$ measures any additional damage when both occur together.

\begin{table}[!htb]
\centering\small
\begin{tabular}{@{}lcc@{}}
\toprule
 & Many tail patches & Few tail patches \\
\midrule
Equal class influence & balanced condition (reference) & S: support only \\
Head-favoured influence & P: prior only & r: ratio condition (both) \\
\bottomrule
\end{tabular}
\caption{Four training conditions separate the effect of unequal class influence (prior) from the effect of fewer tail examples (support) at each $\rho$.}
\label{tab:conditions}
\end{table}

\noindent The two tasks break down differently. On TCGA-UT the prior carries the damage at moderate ratios (0.73 of 1.01 pp at $\rho=20$) while thin support alone costs nothing; at $\rho=100$ the two compound, with prior 1.20, support 0.53 and an interaction of 1.18 pp. On BRACS both factors are material and add up without resolved interaction: prior 4.47 pp and support 3.29 pp at $\rho=100$. Thin support alone thus costs BRACS more than the entire imbalance damage on TCGA-UT.

\section{Dependence on tail-class identity}

Damage depends on which classes end up in the tail, not only on $\rho$. On BRACS, rotating every class through every rank shows that easy classes cost most when rare: damage at $\rho=100$ ranges from 5.73 pp (UDH in the tail) to 9.99 pp (invasive carcinoma), because a class with high balanced recall has the most to lose. On TCGA-UT, a class is especially vulnerable when it receives few patches but a class it is often mistaken for receives many. A score fixed before testing new class orders used this pattern to predict their damage. The order ranked most damaging costs 4.51 pp, the least damaging 1.34 pp, and random orders 2.91 pp (Figure~\ref{fig:order}). Reversing the most damaging order keeps the same rank gaps between pairs of classes but lowers damage by 1.11 pp [0.47, 1.73]. Damage is therefore greatest when the class prone to being mistaken for another is the rarer one.

\begin{figure}[!htb]
\centering
\begin{tikzpicture}
\begin{axis}[width=0.6\textwidth,height=3.4cm,xmin=0.8,xmax=5.3,ymin=0.5,ymax=6.5,
...
\end{axis}
\end{tikzpicture}
\caption{TCGA-UT damage at $\rho=100$ for different class-to-rank assignments, with 95\% intervals.}
\label{fig:order}
\end{figure}

\section{Encoder comparison}
Refitting the prior, support and ratio conditions on identical patients with UNI2-h \citep{chen2024uni} instead of Virchow2 leaves the picture intact. On TCGA-UT, UNI2-h starts 1.78 pp higher and loses 0.86 pp less at $\rho=100$ (97.5\% interval $-1.33$ to $-0.40$), a resolved but sub-1-pp difference driven by smaller support and interaction damage. On BRACS the difference is unresolved and the tail classes collapse to about 9\% recall with both encoders.

\section{Implications for mitigation}
\begin{itemize}\setlength{\itemsep}{1pt}
\item Rebalancing the training objective removes the prior damage but cannot restore thin support. On BRACS, 3.3 pp remain at $\rho=100$; recovering them needs more tail patches or patients, or methods that improve tail-class estimation.
\item Most calibration damage is removed by the global confidence correction, so a mitigation method must beat a corrected baseline to add value in probability quality.
\item Dataset and class order change the damage by a factor of 2--3 at the same $\rho$; evaluations should randomize class order and report both datasets.
\end{itemize}
...
\item \textbf{Class influence versus tail support:} Reports 27 and 28, in \path{01_cause/tcga_ut/27_prior_vs_support/report/} and \path{01_cause/bracs/28_prior_vs_support/report/}, show the paired prior-only and support-only conditions, their interaction, and its uncertainty on each dataset.
