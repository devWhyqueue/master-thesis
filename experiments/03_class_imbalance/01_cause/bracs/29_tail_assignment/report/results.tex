...
FEA, ADH $-$ rest & & & $-$0.34 [$-$1.32, 0.78] & $-$0.85 [$-$2.11, 0.52] \\
\bottomrule
\end{tabular}
\caption{The damage grows with the balanced-arm recall of the class in the tail, and the atypical classes are not the costly ones. Tail damage $D_\rho(c)$ is the balanced-accuracy loss against the balanced arm, in points with paired 95\% bootstrap intervals, over the 30 fits in which class $c$ occupies the tail. Classes are ordered by their patient-macro recall in the balanced arm, in percent; confusability is the off-diagonal confusion mass into and out of the class in percent of all balanced-arm test predictions. The last row is the pre-specified contrast of the atypical classes against the other five.}
\label{tab:damage}
\end{table}

\noindent The pre-specified contrast does not support the overlap reading. With FEA or ADH in the tail, the damage is 0.34 points ($-1.32$ to 0.78) smaller than with one of the other five classes at $\rho=10$ and 0.85 points ($-2.11$ to 0.52) smaller at $\rho=100$. Neither contrast is resolved, and both intervals reach beyond one point below zero, so a smaller damage for the atypical classes is as compatible with the data as a zero difference, whereas a damage larger by more than 0.78 points is not. The two atypical classes also do not behave alike: ADH is among the cheapest tail classes at both ratios, while FEA is among the most costly. Instead, the ordering follows the balanced-arm recall. The Spearman correlation between r1 recall and tail damage is 0.79 at $\rho=10$ and 0.96 at $\rho=100$, where only UDH and ADH swap places. The correlation with confusability is 0.18 and $-0.29$.
...
Of the two class properties, only the headroom orders the damage. The own loss of the tail class correlates with its balanced recall at 0.64 at $\rho=10$ and 0.89 at $\rho=100$, and with confusability at 0.39 and 0.07. The confusability measure does not single out the atypical classes: PB and N are the most confused classes of the balanced arm, with 23.7 and 16.3 percent of all predictions, while ADH and FEA lie at 7.4 and 9.0 percent. The least confused class, IC, is the only one that keeps a substantial share of its recall in the tail.
...
The confusability measure pools patch-level confusion counts and therefore gives classes with many test patches more weight; it does not capture overlap as a pathologist would rate it, and the grouping of FEA and ADH as the overlapping classes rests on morphology rather than on a measured property. The study covers two ratios and one exponential profile. All results are conditional on ten patients per class, frozen Virchow2 features, multinomial logistic regression, BRACS, and three overlapping splits with about 24 test patients each.
