"""Command-line entry point for the tail-class assignment experiment."""
...
from assignment.analyze import run_analyze
from assignment.fit import run_fit_shard, shard_count
...
def _parser() -> argparse.ArgumentParser:
    """Create the assignment command-line argument parser."""
    parser = argparse.ArgumentParser(description="Tail-Class Assignment Experiment CLI")
...
def cmd_fit(args: argparse.Namespace) -> None:
    """Fit every pending shifted-tail arm of one (split, draw) shard."""
    run_fit_shard(load_config(args.config), args.shard_index)


def cmd_analyze(args: argparse.Namespace) -> None:
    """Pool reused and new arm accuracy, write analysis/diagnostics, and the figures."""
