"""Report stage: snapshot this dataset's precheck, pilot diagnostics, and main analysis into one
JSON the hand-written protocol/results ``.tex`` would cite numbers from. Every other experiment in
this codebase writes its report by hand from ``analysis.json``/``diagnostics.json`` directly; this
stage only saves a second look-up from walking three separate files to one.
"""
