"""Command-line entry point for the class-separation intervention experiment (exp-34)."""
...
from separation.analyze import run_analyze
from separation.fit import main_shard_count, pilot_shard_count, run_fit_shard
from separation.gate import run_gate
from separation.precheck import run_precheck
from separation.report import run_report
...
def _parser() -> argparse.ArgumentParser:
    """Create the separation-intervention command-line argument parser."""
    parser = argparse.ArgumentParser(
        description="Class-Separation Intervention Experiment CLI"
    )
...
def cmd_precheck(args: argparse.Namespace) -> None:
    """Compute this dataset's separation index J, and its frozen alpha once the peer is ready."""
...
def cmd_analyze(args: argparse.Namespace) -> None:
    """Pilot: run the five stopping gates. Main: pool B/P/S/R and the primary effect."""
...
def _build_stages(config: dict, chained: bool) -> dict:
    ...
    fit_pilot = _job(
        ...
        array_size=pilot_shard_count(dataset),
    )
    analyze_pilot = _job(
        config,
        "analyze_pilot",
    ...
    fit_main = _job(
        ...
        array_size=main_shard_count(dataset),
    )
    analyze_main = _job(
        config,
        "analyze_main",
