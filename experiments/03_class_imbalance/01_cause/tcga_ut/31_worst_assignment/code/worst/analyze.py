"""Analyze stage: pool exp-25/exp-30 reused arms with the four new-order arms, compute the
pre-specified BA-damage contrasts, the Spearman(S, D) check, the per-class recall-change table,
and confusion-pair rank gaps. Writes analysis.json, diagnostics.json, and two figures.
"""
...
from worst import EXP30_ARMS, NEW_FIT_ARMS, REUSED_ARMS
from worst.figures import damage_vs_score_figure, pair_gap_figure
from worst.order import derive_orders, score_inputs
from worst import report
...
def _write_figures(
    ...
    points = {
        ...
        for key in ("easy", "hard", "sep", "flip", "tog")
    }
    ...
    gap_sep, change_sep, gap_tog, change_tog = report.pair_gap_series(
        p["orders"], r["partner"], r["recall_change"], p["names"]
    )
    pair_gap_figure(
        gap_sep, change_sep, gap_tog, change_tog, figures / "pair_gap_r100.pdf"
    )


def _compute(p: dict[str, Any]) -> dict[str, Any]:
    """Every endpoint, check, and table the diagnostics/figures are built from."""
