"""Fit stage: exp-31's sep/flip/tog r100 and sep r10 arms of one (split, draw) shard.

r1/r10/r100/N are reused unchanged from exp-25's stored outputs; easy_r100/hard_r100 are reused
unchanged from exp-30's stored outputs. Only the four new-order arms are fit here.
"""
...
from worst import ARM_ORDER, ARM_RATIO, NEW_FIT_ARMS
from worst.order import derive_orders, order_perm
...
def _fit_new_arm(
    ...
    _fit_arm(
        ...
        evals,
        draw_idx,
        data_arm=f"r{ARM_RATIO[arm]}",
    )


def run_fit_shard(config: dict[str, Any], shard_index: int) -> None:
    """Fit every pending sep/flip/tog/sep_r10 arm of one (split, draw)."""
