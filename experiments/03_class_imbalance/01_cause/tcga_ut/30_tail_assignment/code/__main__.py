"""Command-line entry point for the tail-class assignment relevance experiment."""
...
from permutation.analyze import run_analyze
from permutation.fit import run_fit_shard, shard_count
...
def _parser() -> argparse.ArgumentParser:
    """Create the permutation command-line argument parser."""
    parser = argparse.ArgumentParser(
        description="Tail-Class Assignment Relevance Experiment CLI"
    )
...
def cmd_fit(args: argparse.Namespace) -> None:
    """Fit every pending easy/hard-tail r100 arm of one (split, draw) shard."""
    run_fit_shard(load_config(args.config), args.shard_index)


def cmd_analyze(args: argparse.Namespace) -> None:
    """Pool reused and new arm accuracy, fit the model, write analysis/diagnostics/figures."""
