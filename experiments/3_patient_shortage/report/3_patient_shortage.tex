...
\label{eq:lambda}
\end{equation}
\noindent Defining the penalty relative to mean loss avoids ambiguity from software conventions that scale regularization with sample count. The stopping tolerance follows the same convention: convergence requires the largest absolute gradient component of Equation~\eqref{eq:linear} to fall below \(10^{-8}\). A solver that minimizes the summed rather than the mean loss therefore receives this tolerance multiplied by \(N\); applying \(10^{-8}\) to the summed objective would instead demand a mean-scale gradient below double-precision resolution and could not be met at any iteration count. Fit the convex objective with deterministic zero initialization and a full-batch solver to numerical convergence. Record solver, precision, stopping tolerance, iteration count, objective, and convergence status. Numerical retries may increase the iteration limit without changing the objective, candidate grid, or data. An unresolved failure makes that candidate unavailable across all three splits for its selection unit; record the failure and do not interpret the resulting search as complete. If no candidate converges throughout, that readout is unavailable.
