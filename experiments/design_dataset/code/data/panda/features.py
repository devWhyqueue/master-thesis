"""Extract frozen Virchow2 features for one PANDA slide (array task).

Features are seed-independent: the capped tile set and its ordering are the same
across split seeds, so extraction reads the reference WSI manifest and writes one
``<slide>.pt`` file reused by every seed. One SLURM array task handles one slide.
"""
...
def main() -> None:
    ...
    slides = sorted(manifest["slide_id"].astype(str).unique().tolist())
    ...
    if feature_path.exists():
        ...
        return
    rows = manifest[manifest["slide_id"].astype(str) == slide_id].sort_values(
        "feature_index"
    )
