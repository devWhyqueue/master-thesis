...
- Datasets: BRACS and TCGA-UT (02–05); TCGA-UT only (06–17). Readout: logistic regression unless noted.
...
| 17 | patient_directions | Is between-patient whitening a separate channel from centre correction, and does a cohort's own basis work? Arms whiten real cohorts along the pool basis (RW) or the cohort's own basis (RWc); R/C/CW reused from exp-16 | Whitening share 0.25 [0.21, 0.29] of the 5→20 gap on real cohorts (not a ceiling). Additive: interaction −0.52 to −0.09 pp per G, S_C + S_W = 1.00 vs S_CW 0.96. Cohort basis gains 2.26/1.85/1.27 pp at G = 5/10/20 (about half of the pool basis) but removes only 0.11 of the gap. Smallest κ chosen in 174/180 fits (grid boundary). |

## Open thread

Exp-16/17 found the mechanism: class-centre error (first moment) carries about three quarters of the benefit of more patients, and whitening along population between-patient directions (second moment) a further quarter; the two are close to additive (exp-17) and together close the gap. Pool centres and the pool basis use all training patients, so this is a mechanism, not a remedy. Whitening along the cohort's own basis is a partial remedy (+1.3–2.3 pp, only 0.11 of the gap). The κ grid did not bracket the optimum for real cohorts (smallest factor chosen). Next: (1) mitigation without extra labelled patients: centre shrinkage and direction estimates pooled across classes, datasets, or unlabelled slides, plus C5 with population patient offsets; (2) predict accuracy at untested G (e.g. 40) and on BRACS from the no-fit centre risk ρ_G = Σ_c Σ_{k≠c} (w_c − w_k)ᵀ Σ_c (w_c − w_k) / G. One patient count per classifier is the standard design.
