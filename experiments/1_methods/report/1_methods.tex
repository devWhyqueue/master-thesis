...
\paragraph{Thesis variant.}
The method uses the \num{512}-dimensional hidden representation of the shared MLP head and is reported as \emph{semantic-scale weighted CE (DSB-CE-1-inspired)}. To isolate diversity from support, every refresh draws the same number of cases and patches per class.

Beginning after four fifths of the update budget $U$, $R$ matched draws are evaluated once per epoch and their volumes are averaged:
\begin{equation}
  S'_{c,t}=\frac{1}{R}\sum_{r=1}^{R}S'_{c,t,r},
  \label{eq:semantic-volume-avg}
\end{equation}
\noindent Averaging volumes rather than covariances preserves the matched budget of each draw. If the full refresh is too expensive, $R=1$ is used and reported. Unlike the source's recent-minibatch pool, this estimator cannot recover class frequency through unequal pool sizes.
