...
Because Equation~\eqref{eq:isw-ce} differs from class-balanced CE only in substituting $G_c$ for $n_c$, a difference between the two is attributable to the change of counting unit alone. At a matched $\beta$ the two coincide exactly when $G_c=n_c$ for every class, that is when every example is its own case; unlike a power-law weight, a proportional deflation $n_c=\overline m\,G_c$ with a common patches-per-case factor does \emph{not} cancel in the normalization, because the transform is not homogeneous. That non-homogeneity is what keeps the rule inside the support signal: a cohort of $[5,5,5]$ cases and one of $[50,50,50]$ produce different weights, whereas any normalized power law in the counts would return the same weights for both and would read a relative case-count profile instead (Section~\ref{sec:support-signal}). The two rules diverge whenever classes differ in how concentrated their material is---a class whose patches come from a handful of heavily tiled cases is downweighted relative to one whose equally many patches come from many cases, although the two are indistinguishable to $n_c$.
