...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[
    ...
    \node[method, below=0.38cm of loss] (lossMethods) {Weighted CE\\Class-balanced CE\\Independent-support CE\\Focal loss\\Pilot-difficulty CE\\AREA (patch)\\Semantic-scale CE (patch)\\Logit adjustment\\CFAL (patch)};
...
Four signals suffice to describe the tested roster (Table~\ref{tab:signals}): prevalence, support, difficulty, and diversity, with support splitting into a nominal and an independent reading. They are stated as properties of a class or example, independently of the intervention that acts on them.
...
\begin{table}[ht]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.15\linewidth}>{\raggedright\arraybackslash}p{0.44\linewidth}>{\raggedright\arraybackslash}X@{}}
    ...
    Diversity & How much within-class variation the observed examples of $c$ actually exhibit & Feature-manifold volume $S'_c$ (semantic scale), effective area $A_c$, effective rank $\mathrm{erank}_c$ \\
    \bottomrule
  \end{tabularx}
  \caption{Signals a mitigation method can use to decide that a class or example needs help. Prevalence is relative and support is absolute, whether counted nominally or in independent units; difficulty and diversity are properties of the data rather than of the class-size profile.}
  \label{tab:signals}
\end{table}

\subsubsection{Prevalence}
Prevalence is the share $\pi_c$ of class $c$ in the training distribution, together with the monotone summaries derived from it: the support rank of a class and the head-to-tail imbalance ratio $\rho=\max_c n_c/\min_c n_c$. It is a relative quantity. Rescaling every class count by a common factor leaves all prevalences unchanged, so a prevalence-aware rule behaves identically on a small and a large copy of the same class profile. It underlies inverse-frequency weights, class-balanced samplers, and prior-corrected decision rules alike, all of which shift optimization or the decision rule toward a more class-balanced target than the observed one. A uniform target is the endpoint of that family rather than what every member implements: the strength controls $s$ and $\tau$ used below reach it only at full correction, and intermediate values rebalance partially.
...
\begin{equation}
  ...
  \label{eq:neff}
\end{equation}
\noindent where $\overline m_c=n_c/G_c$. The two limits make the quantity readable: with $\operatorname{ICC}_c=0$ the class retains its full count $n_c$, and with $\operatorname{ICC}_c=1$ it collapses to $n_c/\overline m_c$, one observation per contributing case. Equations~\eqref{eq:icc}--\eqref{eq:neff} adjust a patch count for redundancy and are named \emph{redundancy-adjusted effective support} throughout. The quantity is read as an approximation rather than as an exact information content: distinct case and slide counts stay the primary independent-support measures, $\operatorname{ICC}_c$ depends on which per-example score it is estimated from, and $\overline m_c$ enters a design effect derived for equal clusters, which understates the correction when cluster sizes are spread out. It is therefore reported with a sensitivity analysis over the cluster-size distribution, substituting the size-weighted mean cluster size $\sum_g m_{gc}^2/n_c$ for $\overline m_c$ and, as a second variant, inflating the design effect by the coefficient of variation $\mathrm{CV}_c$ of the cluster sizes, with classes whose ordering changes under either substitution flagged. The size-weighted mean is the natural retained quantity because it is the cluster size an average \emph{observation} belongs to, which is what the design effect asks for; the correction $m_{0,c}$ of Equation~\eqref{eq:icc} is deliberately not reused here, since it is defined by its role in the ANOVA estimator of $\operatorname{ICC}_c$ and transferring it into the design effect would require a justification the sources cited for either quantity do not supply \citep{kish1965survey,rutterford2015methods}.

The two subtypes are what a method is read against. A method reads \emph{nominal support} when $n_c$ enters its rule absolutely rather than as the share $\pi_c$, and \emph{independent support} when a unit count $G_c$ or $N_{\mathrm{eff},c}$ does. Class-balanced CE models diminishing returns as a function of the raw count alone through $E_c(\beta)$ in Equation~\eqref{eq:cb-effective-number}, and CFAL reuses the same construction; both are nominal-support methods, which separates them from the prevalence-driven methods, but neither asks how many independent units those examples came from. AREA is superficially close, since its effective area in Equation~\eqref{eq:area-effective} deflates a count by the same design-effect algebra \citep{chen2023area}, but it measures redundancy among a class's features in the learned representation rather than over the case an example was cut from, which places it under diversity. Independent support is read by independent-support weighted CE (Section~\ref{sec:isw-ce}), a construction of this thesis that substitutes $N_{\mathrm{eff},c}$ for $n_c$ inside the power-law weight of class-weighted CE.
...
\begin{table}[!htbp]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.23\linewidth}>{\raggedright\arraybackslash}p{0.42\linewidth}>{\raggedright\arraybackslash}X@{}}
    ...
    Semantic-scale weighted CE & Diversity, through the feature-manifold volume $S'_c$; the source's inter-class crowding factor $W_c$ is dropped & Per-example class-dependent loss weight \\
...
Counted by signal, eight interventions consult prevalence alone, and SC-MIL adds a partial case through class-aware batching. Two read difficulty: focal loss at example level and pilot-difficulty weighted CE at class level. Two read nominal support through the effective number, which CFAL combines with an example-level difficulty factor. Two read diversity, AREA and semantic-scale weighted CE, and one reads independent support, the construction of this thesis, since none of the collected methods counts the units a class's examples came from.
Six interventions---class-weighted CE, class-balanced CE, independent-support weighted CE, pilot-difficulty weighted CE, AREA, and semantic-scale weighted CE---share a single intervention level: all six apply a mean-one normalized class weight to unmodified CE, and they differ in the per-class score they read, namely $n_c$, $E_c(\beta)$, $N_{\mathrm{eff},c}$, $d_c$, $A_c$, and $S'_c$. Holding the intervention level fixed substantially reduces the mechanistic differences between them but does not eliminate them. Three residual differences are recorded rather than assumed away. The weights are formed by different transforms of their score: a power law $x^{-\tau}$ for class-weighted, independent-support weighted, AREA, and semantic-scale weighting, a saturating effective-number map for class-balanced CE, and $(d_c+\epsilon)^{\tau}$ for pilot-difficulty weighting. The four fixed rules compute their weights once before the first update, whereas the two diversity rules re-estimate their score from the evolving representation and therefore also differ in temporal adaptation. And the two dynamic rules act only after a deferred start at four fifths of the update budget, so they leave part of training unweighted. The four fixed rules are consequently the closest thing here to a signal-only contrast, and the diversity rules are read against them as differing in signal and schedule together; a strictly signal-only family, in which every score entered one generic weighting function under one update schedule, would require a second controlled roster and is not constructed here. Contrasts among the prevalence-aware methods vary the intervention at a fixed signal, these six vary the signal at a largely fixed intervention, and difficulty and diversity each admit a within-signal contrast in addition. The quantities $d_c$, $N_{\mathrm{eff},c}$, and $\mathrm{erank}_c$ are also carried forward as predictors in their own right, independently of whether a method acting on them succeeds.
...
\begin{equation}
  ...
  \label{eq:isw-ce}
\end{equation}
\noindent The exponent $\tau\geq0$ is the single imbalance control, and $\tau=0$ recovers ordinary CE. Because Equation~\eqref{eq:isw-ce} differs from class-weighted CE only in substituting $N_{\mathrm{eff},c}$ for $n_c$, a difference between the two is attributable to the redundancy adjustment alone: the two rules coincide when every class draws its examples from equally many independent units and diverge exactly when a class's examples concentrate in few cases. Both remain power-law corrections, which separates this method in turn from the saturating count correction of Section~\ref{sec:cb-ce}.
...
\section{Summary}
\label{sec:summary}
This report defines the no-mitigation references (CE and MIL CE) and the retained interventions: balanced sampling, class-weighted CE, class-balanced CE, independent-support weighted CE, focal loss, pilot-difficulty weighted CE, AREA, semantic-scale weighted CE, logit adjustment in train-time and post-hoc form, the soft-F1 and soft-MCC hybrids, CFAL, OKO, cRT, SC-MIL, and MDE-inspired dual-expert MIL. Balanced Softmax is recorded as related prior-correction literature rather than an additional tested method. Read along the signal axis---prevalence, support, difficulty, diversity---the roster stays anchored on prevalence while carrying a contrast for each remaining dimension: class-balanced CE isolates the nominal-support reading, a diminishing-count correction, from the prototype and affinity components CFAL bundles with the same effective number, and pilot-difficulty weighted CE supplies a class-level difficulty signal distinct from the example-level confidence used by focal loss. Diversity is read by two rules that measure the space a class spans in the current representation and differ only in the statistic---AREA through the effective area of its feature correlations, semantic-scale weighted CE through the volume of its feature manifold, with the source's inter-class crowding factor dropped so that the signal stays within-class---and independent support by the one construction of this thesis, independent-support weighted CE, which is labelled as such throughout; together with the difficulty-based rules they place all four signals on the same class-weighting mechanism, at a fixed intervention level but not an identical weighting transform or update schedule. The definitions make the tested adaptations, output semantics, and ablations explicit, which is what the evaluation of their discrimination, probability quality, and mitigation recovery presupposes.
