...
To avoid mixing support with classification difficulty, the ICC is estimated from a classifier-independent score: the projection of each frozen Virchow2 embedding onto the leading principal direction of a PCA fitted jointly across classes on a fixed class-balanced reference. This scalarization and the exchangeable within-case correlation model are approximations, so $N_{\mathrm{eff},c}$ is reported alongside $G_c$ as a descriptive statistic and does not enter the training objective. Because a full embedding pass over every condition would repeat the same cost as the diversity draw at far larger scale, the ICC estimate is drawn from a capped sample of at most 200 cases and 200 patches per case per class, a deviation from Equation~\eqref{eq:icc} as written that trades estimation precision for tractability on a purely descriptive statistic.
...
\begin{table}[!htbp]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.25\linewidth}>{\raggedright\arraybackslash}p{0.32\linewidth}>{\raggedright\arraybackslash}X@{}}
    ...
    Focal loss & Nominal support ($n_c$) and example difficulty ($p_y$) & Loss weight \\
...
Focal loss reduces the contribution of confidently classified examples and concentrates optimization on examples that remain difficult. With focusing parameter $\gamma$ and class weight $\alpha_y$, the multiclass focal objective is
\begin{equation}
  \mathcal{L}_{\mathrm{focal}}(\mathbf{z},y) =
  -\alpha_y(1-p_y)^{\gamma}\log p_y.
  \label{eq:focal}
\end{equation}
The reference default is $\gamma=\num{2.0}$ for both regimes; $\gamma$ is the tuned control selected on validation while the rest of the training protocol is held fixed. $\alpha$ is fitted alpha-balanced: the derived, not tuned, mean-one inverse-frequency weight of Equation~\eqref{eq:weighted-ce} at $s=1$. The method is relevant to imbalance because many minority-class examples can remain hard under a classifier dominated by head-class gradients; combined with $\alpha$-balancing it addresses both difficulty and class frequency, rather than difficulty alone.
...
Classes with smaller volume receive larger mean-one weights. Let $r_c=S_c'^{-\tau}$ for a class with a valid volume estimate, and $r_c=1$ (no reweighting) for a class without one:
\begin{equation}
  w_c^{\mathrm{SSB}}(\tau)=
  \frac{r_c}
       {\frac{1}{C}\sum_{j=1}^{C}r_j},
  \qquad
  \mathcal{L}_{\mathrm{SSB}}(\mathbf z,y;\tau)=-w_y^{\mathrm{SSB}}(\tau)\log p_y.
  \label{eq:ssb-weight}
\end{equation}
\noindent The exponent $\tau\geq0$ is the only tuned control: $\tau=0$ recovers ordinary CE and $\tau=1$ gives inverse-volume weighting. Because the volumes are refreshed as the representation changes, this remains a dynamic weighting method.
...
A draw with fewer than two usable features for a class is excluded. If no valid draw remains, or the averaged volume is zero, the previous estimate is retained; a class with no valid estimate at all -- at the first refresh, or in every refresh since -- keeps unit weight ($r_c=1$ in Equation~\eqref{eq:ssb-weight}) rather than a maximal fallback weight, since a degenerate draw carries no diversity evidence to reweight by. Such events are recorded. The method is evaluated only for patch classification because matched WSI-level class sets are too small for stable covariance estimates.
