...
The score the ICC is read from must be \emph{classifier-independent}, since $\operatorname{ICC}_c$ inherits whatever that score measures: a score read off classification performance---the CE loss, the true-class probability $p_y$, a probe margin---would make $N_{\mathrm{eff},c}$ a mixture of support and difficulty. The score used is the projection of an example's frozen Virchow2 embedding onto the leading principal direction of a PCA fitted over all classes jointly on the class-balanced development reference (Section~\ref{sec:signal-cohorts}), which records where an example sits along the encoder's dominant axis of variation and says nothing about how well any classifier separates it. It is not label-free in the strict sense---labels determine the class-balanced composition of the manifest the PCA is fitted on, and which examples are pooled inside a class---but no fitted map from features to labels enters it, which is the property the ICC requires. That choice is also why $N_{\mathrm{eff},c}$ describes rather than intervenes. A projection onto one principal direction is a defensible summary of embedding variation but not a canonical measure of pathological evidence: the direction can carry stain, scanner, tissue quantity, or morphology in unknown proportion, and a different scalarization can reorder the classes. The exchangeable model behind Equation~\eqref{eq:neff} adds a second approximation, treating every pair of examples inside a cluster as equally correlated regardless of where on the slide they were cut. Both qualifications are acceptable in a reported statistic and would be awkward inside a training rule, so $N_{\mathrm{eff},c}$ is reported alongside $G_c$ to show how much redundancy sits beneath the unit count, and classes whose $N_{\mathrm{eff},c}$ ordering is unstable across alternative classifier-independent scalarizations are flagged where the statistic is used.

The two subtypes are what a method is read against. A method reads \emph{nominal support} when $n_c$ enters its rule absolutely rather than as the share $\pi_c$, and \emph{independent support} when a unit count does. Class-balanced CE models diminishing returns as a function of the raw count alone through $E_c(\beta)$ in Equation~\eqref{eq:cb-effective-number}, and CFAL reuses the same construction; both are nominal-support methods, which separates them from the prevalence-driven methods, but neither asks how many independent units those examples came from. Independent support is read by independent-support weighted CE (Section~\ref{sec:isw-ce}), a construction of this thesis that substitutes $G_c$ for $n_c$ inside the effective-number weight of class-balanced CE, so that the two support subtypes are read through one and the same transform. The transform has to be the saturating one for the substitution to stay inside the support signal: a power-law weight $G_c^{-\tau}$ normalized across classes is homogeneous of degree zero in the counts, so it returns identical weights for a cohort of $[5,10,20]$ cases and for one of $[50,100,200]$, and therefore reads the relative case-count profile---an independent-unit prevalence---rather than an absolute magnitude. Equation~\eqref{eq:cb-effective-number} is not homogeneous, so at a fixed $\beta$ those two cohorts generally receive different weight ratios even though their case prevalences agree.

What the transform cannot do is act on the overall \emph{level} of support, and the reason is the normalization rather than the score. The effective-number score is absolute, but every weighting rule in this roster divides by the classwise mean, so any factor common to all classes cancels and the intervention acts only on \emph{between-class} differences in the score. A cohort of $[5,5,5]$ cases and one of $[50,50,50]$ consequently both yield $w_c=1$ for every class, under the saturating transform exactly as under a power law, and the two trained objectives are identical. If every class is equally support-poor there is no between-class difference for a normalized class weight to correct, and reweighting cannot remedy a globally low-data regime; the absolute reading of the signal shows up only where classes differ in magnitude, not where they are jointly small.
...
\begin{table}[!htbp]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.23\linewidth}>{\raggedright\arraybackslash}p{0.42\linewidth}>{\raggedright\arraybackslash}X@{}}
    ...
    Independent-support weighted CE & Independent support, through the effective number $E_c(\beta)$ evaluated at the distinct case count $G_c$ & Per-example class-dependent loss weight \\
    ...
    CE + soft F1 & Prevalence, through balanced sampling; the equal classwise weighting of the macro term is class-symmetric rather than a prevalence measurement & Sampler and auxiliary objective \\
...
\label{sec:isw-ce}
This method supplies the independent-support subtype of Section~\ref{sec:support-signal}, and its counterpart is class-balanced CE in both readings at once. Taxonomically the two occupy the two subtypes of one signal, so the pair asks whether support counted in independent units behaves differently from support counted in raw examples. Mechanistically they apply the identical saturating transform of Equation~\eqref{eq:cb-effective-number} and differ only in the count fed into it, so at a matched $\beta$ the pair isolates the change of counting unit with the transform held fixed---a cleaner contrast than a comparison against class-weighted CE would give, since that rule differs in transform and in signal type at once. Class-weighted CE and class-balanced CE both read $n_c$, and no collected method asks how many cases those examples were cut from; that raw count in pathology overstates what a class carries, because patches drawn from one case share its staining, scanner, preparation, and much of its morphology (Section~\ref{sec:signal-axis}). Independent-support weighted CE is constructed for this thesis to supply that channel, and the distinction it operationalizes is elementary: nominal support asks how many training examples a class has, independent support how many independent cases produced them.

The rule keeps the functional form of Equation~\eqref{eq:cb-weight} and changes only the count the effective number is evaluated at. With the distinct case count $G_c$ of Section~\ref{sec:support-signal},
\begin{equation}
  ...
  \qquad
  \mathcal{L}_{\mathrm{ISW}}(\mathbf z,y;\beta)=-w_y^{\mathrm{ISW}}(\beta)\log p_y,
  \label{eq:isw-ce}
\end{equation}
\noindent again normalized to mean one across classes. The single imbalance control is $\beta$, exactly as in Section~\ref{sec:cb-ce}: $\beta=0$ sets every effective number to one and recovers ordinary CE, and $\beta\rightarrow1$ approaches inverse-case weighting. Its informative range differs, however, because the counts differ in magnitude. Saturation sets in once the count exceeds roughly $(1-\beta)^{-1}$, so the patch-scale reference default $\beta=\num{0.999}$ leaves case counts in the tens entirely in the linear regime, where $E_c^{\mathrm{ind}}\approx G_c$; the values that make the transform saturate in case units are correspondingly smaller, and $\beta$ is selected on validation over a grid matched to the case scale rather than shared with class-balanced CE.

That tuning choice splits the comparison against class-balanced CE into two analyses, reported separately. The \emph{validation-tuned} comparison puts the best selected configuration of each rule against the other; it answers which support rule performs better in practice, but each rule then carries its own $\beta$, so the observed difference reflects the counting unit and the saturation scale together and no single-factor attribution is claimed for it. The \emph{matched-$\beta$} contrast holds $\beta$ at a common value across the two rules, leaving $n_c$ versus $G_c$ as the only difference between the objectives; only this arm supports statements about the change of counting unit alone. The matched-$\beta$ arm is run at the $\beta$ values of the shared portion of the two grids and is reported as a controlled contrast rather than as the headline result, since a $\beta$ that suits one count scale need not suit the other.

Because Equation~\eqref{eq:isw-ce} differs from class-balanced CE only in substituting $G_c$ for $n_c$, a difference between the two in the matched-$\beta$ arm is attributable to the change of counting unit alone. Since both weights are normalized to mean one, the two rules issue identical weights at a matched $\beta$ whenever the two effective-number profiles are proportional,
\begin{equation}
  E_c^{\mathrm{ind}}(\beta)=\kappa\,E_c(\beta)\quad\text{for all }c
  \label{eq:isw-coincide}
\end{equation}
\noindent and some common $\kappa>0$; the case $G_c=n_c$ for every class, where every example is its own case, is the special case $\kappa=1$. The condition is not vacuous in the other direction: any cohort whose $n_c$ profile and whose $G_c$ profile are both flat satisfies it with both rules returning $w_c=1$, and more generally the two rules agree wherever the deflation from patches to cases happens to preserve the shape of the effective-number profile. What does \emph{not} suffice is a proportional deflation of the counts themselves: $n_c=\overline m\,G_c$ with a common patches-per-case factor leaves Equation~\eqref{eq:isw-coincide} violated in general, because the transform is not homogeneous, which is what keeps the rule inside the support signal rather than reading a relative case-count profile (Section~\ref{sec:support-signal}). The two rules diverge whenever classes differ in how concentrated their material is---a class whose patches come from a handful of heavily tiled cases is downweighted relative to one whose equally many patches come from many cases, although the two are indistinguishable to $n_c$.

Three properties of the signal carry into the method. First, $G_c$ is a property of the training condition rather than of the class, so it is recomputed for each condition exactly as $n_c$ is, in the split fixed by Section~\ref{sec:signal-cohorts}; unlike every other non-prevalence score in the roster it requires no fitted scoring function, no representation, and no labels beyond the class assignment itself, so the weights carry no estimation error that class-balanced CE does not also carry. Second, the redundancy-adjusted effective support $N_{\mathrm{eff},c}$ of Equation~\eqref{eq:neff} is reported alongside the realized weights rather than substituted into them: it says how much within-case redundancy sits beneath each $G_c$ and therefore how much a finer correction could have moved the weights, without making the objective depend on a chosen scalarization of the embedding space. Third, the method is evaluated in the patch regime only, and that restriction is conditional on the cohort rather than categorical: $G_c=n_c$ collapses the rule onto class-balanced CE exactly when the labelled slide is itself the split unit, which requires that no case contribute several correlated slides. It therefore holds for a slide-per-case cohort such as CAMELYON16, where slide-disjoint and case-disjoint splits coincide \citep{queisler2026datasets}, and would have to be reconsidered on a cohort whose cases contribute several slides, since there the WSI-level $G_c$ still separates two classes with equal slide counts.
...
A source-faithful implementation requires a balanced monitor set that is re-evaluated throughout training. \citet{sinha2022classDifficulty} note that this subset may double as the validation or development set, so reusing held-out data for it is consistent with CDB-W rather than excluded by it. The refusal to do so here is a benchmark-design choice: the natural-validation cohort is the only held-out data of that kind, and it is reserved for hyperparameter and checkpoint selection, so feeding it back as a per-epoch training signal would make model selection and training share a cohort across every method in the roster and break the comparison the benchmark is built on. The tested variant therefore uses the matched-support class difficulty $d_c$ of Equation~\eqref{eq:difficulty}, obtained once from the hierarchically matched out-of-fold probe draws and locked before the training conditions are constructed. With a fixed $\epsilon=\num{e-3}$ that only prevents zero weights for a perfectly separated class---three orders of magnitude below the unit range $d_c$ lives on, so it changes no non-degenerate weight appreciably---the mean-one normalized weight is
\begin{equation}
  w_c^{\mathrm{PDW}}(\tau)=
  \frac{(d_c+\epsilon)^{\tau}}
...
Let $M$ be the number of feature vectors one hierarchically matched draw of Section~\ref{sec:diversity-signal} contributes per class---the same in every class by construction, and therefore not $n_c$. Let class $c$ contribute the feature vectors $\mathbf h_1,\ldots,\mathbf h_{M}$ from the representation currently being trained, with class mean $\boldsymbol\mu_c$, and collect the centred features $\mathbf f_i=\mathbf h_i-\boldsymbol\mu_c$ as the columns of $Z_c\in\mathbb R^{d\times M}$. The volume of the region these features span is proportional to the determinant of their covariance, $\det\bigl(\tfrac{1}{M}Z_cZ_c^{\top}\bigr)$, which vanishes whenever the sample covariance is rank-deficient---the ordinary case here, since $M<d$ after the matched-size subsampling required of any diversity statistic. Following the rate-distortion construction the method borrows, the volume is therefore measured at a finite precision $\varepsilon$, which regularizes the determinant and yields the semantic scale
\begin{equation}
  S'_c=\tfrac{1}{2}\log_2\det\Bigl(I+\tfrac{d}{M\varepsilon^{2}}Z_cZ_c^{\top}\Bigr).
  \label{eq:semantic-volume}
\end{equation}
\noindent The precision is carried explicitly here. \citet{ma2023semanticScale} derive the same expression with $\varepsilon$ in it, but their operational formula and pseudocode drop the factor and read $\tfrac{1}{2}\log_2\det\bigl(I+\tfrac{d}{m}Z_cZ_c^{\top}\bigr)$, with $m$ the number of features their pool holds for the class, on the argument that the sphere radius does not change the relative size of the spaces the classes span; the surrounding text nonetheless states $\varepsilon=\num{1000}$. Equation~\eqref{eq:semantic-volume} is therefore the derivation's form rather than the published operational one, with $\varepsilon=1$ recovering the latter exactly. The tested rule takes that value, $\varepsilon=1$, on isotropically rescaled feature coordinates, and the precision is therefore a fixed constant rather than a tuned control (see below). It is written out because the value is only meaningful together with the scale of the features it is applied to, which the published operational form leaves implicit. Equation~\eqref{eq:semantic-volume} aggregates the same eigenvalues that Equation~\eqref{eq:effective-rank} does, through $\sum_i\log_2(1+\tfrac{d}{M\varepsilon^{2}}\lambda_i)$ rather than through their entropy: a class occupying many directions with comparable variance scores high, and one whose variance collapses onto few directions scores low however many examples it contributes.

Raw volume is subject to a marginal effect. Each additional example is increasingly likely to fall in directions the class already spans, so $S'_c$ saturates as the number of features it is computed from grows and understates how differently two classes are supported. The method therefore reads volume relative to the space a class is left by its neighbours. With class centres $\mathbf o_1,\ldots,\mathbf o_C$, let
\begin{equation}
...
The estimator is the second departure. The features entering Equation~\eqref{eq:semantic-volume} are the \num{512}-dimensional hidden representations of the shared MLP head, and $S'_c$ is refreshed once per epoch after a deferred start at four fifths of the update budget $U$, from the hierarchically matched subsample of Section~\ref{sec:diversity-signal}: the same number of cases per class and the same number of patches per case. Each refresh $t$ draws $R$ independent matched subsamples under a seed derived from the refresh index and averages the resulting volumes,
\begin{equation}
  S'_{c,t}=\frac{1}{R}\sum_{r=1}^{R}S'_{c,t,r},
  \label{eq:semantic-volume-avg}
\end{equation}
\noindent so that the weights follow the representation rather than which cases one draw happened to select. Averaging is applied to the volumes and not to the covariances, keeping each draw's determinant an estimate at the matched budget; the isotropic scale $s_t$ below is estimated once per refresh on the pooled features of all $R$ draws. A single draw ($R=1$) is the fallback if the forward passes prove too expensive at the refresh rate used, and the value of $R$ is reported with the results, since it sets how much of the residual small-support variance survives into the weights. The degeneracy rules of Equation~\eqref{eq:ssb-weight} are applied to the averaged $S'_{c,t}$, with a draw that yields fewer than two matched features for a class excluded from its average and the refresh treated as degenerate for that class if no draw remains. The source instead maintains a feature pool of recent minibatches, updated every iteration and reweighting only after a warm-up phase, which makes the estimate depend on how often a class appeared. That dependence is deliberate there: the marginal effect of Equation~\eqref{eq:semantic-relative}'s motivation is part of the source's argument, and the source quantity intentionally combines observed support with feature diversity, so a class that is both scarce and narrow is penalized on both counts at once. The matched estimator used here orthogonalizes the latter from the former, because the signal-isolation experiment this roster is built for requires diversity to vary while support is held fixed---an unmatched pool would let $S'_c$ grow with the number of features it is computed from and recover class size under a different name, collapsing the diversity arm onto the support arm. The adaptation therefore removes a component the source wanted, for a reason the source did not have.

The precision $\varepsilon$ is the remaining departure, and it is a departure in the coordinates rather than in the value: $\varepsilon=1$ exactly as in the source's operational formula, but applied after rescaling the feature coordinates instead of to the raw hidden activations, and not tuned, which leaves $\tau$ as the only control selected on validation. Fixing it explicitly is not pedantry, because $\varepsilon$ enters Equation~\eqref{eq:semantic-volume} through $\log_2\bigl(1+\tfrac{d}{M\varepsilon^{2}}\lambda_i\bigr)$ and therefore decides which eigenvalues fall in the linear regime of the logarithm and which saturate; a different value reweights small against large directions and can reorder the class weights rather than only rescale them. The standardization is a single isotropic scale factor, class-agnostic and re-estimated at every refresh: the hierarchically matched features of all classes are pooled, their pooled covariance $\Sigma_t$ is formed, and every feature is divided by $s_t=\bigl(\tfrac{1}{d}\operatorname{tr}\Sigma_t\bigr)^{1/2}$ before Equation~\eqref{eq:semantic-volume} is evaluated. Only the scale is needed: subtracting a pooled mean would leave every $S'_c$ unchanged, because $Z_c$ is already centred on its own class mean. Re-estimating $s_t$ is what keeps $\varepsilon=1$ comparable across refreshes. A factor frozen at one point would not remove later drift---if the hidden features double in magnitude afterwards, their rescaled coordinates double with them and $\varepsilon$ silently changes meaning---whereas an estimate taken at the same moment as the volumes it rescales divides out whatever global magnitude the representation currently has. That the factor is a scalar and not a per-coordinate vector is deliberate. Per-coordinate standardization $\mathbf h\mapsto D^{-1}(\mathbf h-\boldsymbol\mu)$ is a diagonal transform that reweights the feature dimensions unequally, so a class varying mainly along one coordinate and a class varying mainly along another have their volumes rescaled by different amounts and their relative semantic scales can change; dividing by one scalar rescales every direction alike, so it stabilizes the overall feature magnitude while leaving the geometry and the between-class differences in spread that $S'_c$ exists to measure untouched. Standardizing per class would go further in the same wrong direction, rescaling each class's own axes and erasing part of that total-spread difference, leaving something closer to a purely directional statistic like $\mathrm{erank}_c$. The parameter $\alpha$ has no role once $W_c$ is dropped. The method is evaluated in the patch regime only, since a rare diagnosis contributes too few bag embeddings for a stable covariance estimate at matched size. The paper's second use of semantic scale, as a label-free diagnostic of model bias, is not tested here.
...
The reference defaults are $\lambda=\num{0.1}$, $\beta=\num{0.999}$, and $\gamma=\num{2.0}$ following Mahbub et al.; all three are held fixed and the affinity bandwidth $\sigma$ is the tuned control selected on validation; it sets the metric scale of Equation~\eqref{eq:cfal-affinity} rather than a rebalancing strength, so no value of it recovers ordinary CE. The paper's $\sigma=\num{130}$ was calibrated for un-normalized AlexNet fully-connected activations, whose squared prototype distances are unbounded and can be large. Equation~\eqref{eq:cfal-affinity} instead consumes the L2-normalized \num{512}-dimensional hidden representations and prototypes described above, for which $0\leq\lVert\hat{\mathbf z}_i-\hat{\mathbf w}_c\rVert_2^{2}\leq4$ holds regardless of the dimension of the space. At $\sigma=\num{130}$ every affinity therefore lies within \num{0.031} of one, and the margin term of Equation~\eqref{eq:cfal-margin} sees no usable contrast between the true class and any other. The collapse is a consequence of the unit normalization and not of the embedding dimensionality---the same bound holds for the \num{2560}-dimensional Virchow2 vectors the trunk consumes---so the tested implementation evaluates a $\sigma$ range matched to that bounded geometry. CFAL is evaluated only in the patch regime because the source method assumes instance-level labels rather than weak slide-level bags.

\paragraph{Thesis adaptation.}
Equation~\eqref{eq:cfal-affinity} keeps the Gaussian affinity of \citet{mahbub2024centerFocused} unchanged in form, but the geometry it is evaluated in differs, so the method is reported as \emph{CFAL-inspired}. Two departures are responsible. First, affinities are computed on L2-normalized representations and prototypes, whereas the source applies the Gaussian to un-normalized features; normalization bounds the squared distance in $[0,4]$ and therefore changes both the admissible range of $\sigma$ and the meaning of a fixed margin $\lambda$ on affinities. It is retained because the frozen-encoder setting supplies representations whose global magnitude is neither controlled nor comparable across training conditions, which would otherwise make one $\sigma$ grid invalid across conditions. Second, $\sigma$ is selected on validation rather than fixed at the published value, which is the direct consequence of the first departure. The remaining components---the margin term of Equation~\eqref{eq:cfal-margin}, the effective-number weight $1/E_{y_i}$, the affinity difficulty factor $(1-a_{i,y_i})^{\gamma}$, and the prototype diversity regularizer---follow the source, with $\lambda$, $\beta$, and $\gamma$ at its defaults.
