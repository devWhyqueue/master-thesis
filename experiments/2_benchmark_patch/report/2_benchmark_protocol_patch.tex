...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[note, below=0.55cm of construct] (contrast) {Controlled comparisons share $T$, model family, and update budget;\\only class allocation differs.\\Validation and test data remain unchanged.\\The natural anchor uses all eligible training patches\\and is not size-matched.};
...
The method roster creates many related method-by-condition comparisons. Treating every result as an independent primary test would increase the chance of a false-positive claim, so the benchmark prespecifies a limited confirmatory family whose tests are adjusted jointly. The confirmatory part of RQ2 asks whether signal choice matters when the intervention mechanism is held fixed. It therefore compares the five weighted-CE methods in Table~\ref{tab:roster}, whose weights use prevalence, nominal support, independent support, difficulty, or diversity while leaving the CE mechanism unchanged. Differences among them can consequently be interpreted as evidence about the signal rather than the intervention. Within each dataset, paired tests support the gate-passing comparisons and Holm adjustment controls their joint family-wise error. Other roster members retain the same effect estimates and confidence intervals but remain exploratory. Appendix~\ref{app:testing} specifies the tests, multiplicity family, and protocol revision; Appendix~\ref{app:controls} gives method-specific diagnostic and tuning details.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.27\linewidth}Xl@{}}
    ...
    \bottomrule
  \end{tabularx}
  \caption{Design and training quantities locked before the first definitive fit, with the appendix section that specifies each. Only class allocation within the shared training support $T$ varies across controlled conditions.}
  \label{tab:locked}
\end{table}
...
These tests establish whether each primary method improves on imbalanced CE; they do not directly test one primary method against another. The five raw effects and recovery fractions are therefore reported side by side with the pre-outcome signal profile from RQ1. Signal alignment is interpreted descriptively by relating each method's recovery to the corresponding signal component without ranking unlike shortages on a common scale. The signals have no common natural unit and no prespecified rule reduces them to one dominant-shortage label, so no condition is labelled ``matched'' after inspecting recovery. In particular, significance for one method and non-significance for another is not treated as evidence that the methods differ. A confirmatory claim that one signal outperforms another would require a prespecified direct between-method contrast, which this family does not contain.
...
\begin{description}
  ...
  \item[Intraclass correlation.] The estimated similarity of class margins from patches belonging to the same partitioning-unit or slide cluster.
