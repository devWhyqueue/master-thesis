...
The need for mitigation is usually inferred from a count of examples per class. In pathology, that count is a weak diagnosis of what a class actually lacks. Thousands of patches may come from only a few patients, so a class with abundant patches can still rest on a thin pool of independent cases; patch selection draws round-robin from a per-class case pool under a patient contribution cap, so a severe patch count and the number of distinct cases behind it need not track each other, and a class can be patch-rich yet case-poor, or the reverse. The count is equally silent about how separable a class is under the available representation and about how much within-class variation its examples span. Frequent classes then dominate the empirical objective, while an underserved class may provide too little independent evidence for stable decision boundaries or reliable probabilities \citep{saini2023tacklingClass,zhang2023deepLong,mosquera2024classImbalance}.

Comparing naturally imbalanced datasets cannot identify this effect by itself. Such datasets differ simultaneously in task difficulty, class separability, training volume, label granularity, and evaluation prevalence. The benchmark instead uses training allocation as a controlled lever and asks a narrower counterfactual question: \emph{for the same task and the same total training budget, what changes when support is allocated evenly rather than concentrated in head classes?} A size-matched balanced cross-entropy (CE) model provides the reference. Validation and test cases remain natural and unchanged, so any balanced--imbalanced difference is tied to training allocation rather than to a different evaluation population.

A second question follows from the first, and is the reason allocation is a lever rather than the subject: redistributing support creates more than one kind of shortage --- of raw prevalence, of nominal or independent support, of learnable signal in difficult classes, of representational diversity --- and a mitigation method that reads one signal need not help when a different shortage is the one actually present. Which shortage a condition realizes is therefore measured rather than assumed from its nominal severity. The companion methods report places the tested roster on this signal axis and defines the intervention level at which each method acts \citep{queisler2026imbalanceMethods}; this benchmark is where that axis is tested empirically, not only defined.
...
\end{description}
RQ1 first establishes whether a recoverable problem exists, and which signal(s) characterize it. RQ2 is evaluated only against that demonstrated deficit, and asks whether reading the right signal matters. RQ3 then asks why the size of the deficit and available headroom differ; it is an exploratory association analysis, not another method ranking. RQ1 gates RQ2, and RQ3 is exploratory throughout. The benchmark is scoped to patch classification, which is what makes the signal axis separable in the first place: the round-robin case-pool sampling above decouples nominal from independent support only because a patch redistributes evidence inside a fixed case pool, whereas redistributing labelled slides moves both counts together. Dataset provenance and detailed method objectives remain in the companion reports \citep{queisler2026datasets,queisler2026imbalanceMethods}. The next section turns this sequence into the benchmark's organizing logic.
