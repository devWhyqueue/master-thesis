...
Section~\ref{sec:analysis} therefore produces three outputs from each controlled comparison: whether damage occurred, how large it was, and how much of it was recovered when recovery was defined. RQ1 and RQ2 estimate these quantities within a dataset and task. The remaining question is why they vary across datasets, severities, and tail assignments. Section~\ref{sec:rq3} treats the three outputs as outcomes and relates them to prespecified properties of each comparison.

\section{Explaining Heterogeneity}
\label{sec:rq3}
RQ3 follows this handoff by asking which prespecified properties are associated with damage occurrence, damage magnitude, and mitigation headroom. Controlled comparisons, rather than individual patients, are the observations in this exploratory analysis. For damage occurrence and magnitude, one \emph{controlled comparison} is a dataset, severity, and tail assignment cell; recovery comparisons additionally identify the method and gate. The analysis neither predicts patient outcomes nor creates another method ranking.
...
\begin{table}[ht]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.23\linewidth}>{\raggedright\arraybackslash}p{0.49\linewidth}X@{}}
    ...
    Minimum independent support & How many distinct partitioning units or slides support the scarcest class? & One-predictor sensitivity \\
...
Unique partitioning-unit and slide counts remain the primary support measures. As a secondary sensitivity analysis, within-case redundancy is discounted through the effective support $N_{\mathrm{eff},c}$ defined in the companion methods report \citep{queisler2026imbalanceMethods}, whose intraclass correlation (ICC) is computed here over partitioning-unit or slide clusters within a class.
