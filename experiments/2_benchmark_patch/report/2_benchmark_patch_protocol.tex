...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[route, below left=0.8cm and 0.45cm of contrast] (ba) {Balanced-accuracy deficit\\$D_{\mathrm{BA}}\geq0.01443$ and CI excludes 0};
    \node[route, below right=0.8cm and 0.45cm of contrast] (cal) {Tail macro-NLL deficit\\$D_{\mathrm{cal}}\geq0.18237$ nats and CI excludes 0};
...
Figure~\ref{fig:gate-routing} shows the two routes. The \emph{discrimination gate} opens when the CE balanced-accuracy deficit is at least \num{0.01443} and its paired 95\% confidence interval excludes zero. The \emph{calibration gate} opens when the CE tail-group macro-NLL deficit, $\mathrm{NLL}_{\mathrm{tail}}(\mathrm{imbalanced\ CE})-\mathrm{NLL}_{\mathrm{tail}}(\mathrm{balanced\ CE})$, is at least \num{0.18237} nats with a paired 95\% confidence interval excluding zero. Both rules are computed from CE runs on the same held-out cases before any mitigation method is compared.

Both thresholds are derived, not assumed. The discrimination threshold is $\max(0.01,\,2\sigma_{\mathrm{seed}})$, where the floor \num{0.01} is the support pilot's own balanced-accuracy materiality constant (Appendix~\ref{app:construction-support}) and $\sigma_{\mathrm{seed}}$ is the largest across datasets and splits of the confirmation-seed standard deviation of balanced-condition CE case-macro balanced accuracy; the noise-floor term binds ($\sigma_{\mathrm{seed}}=0.00721$, BRACS). The calibration threshold is \num{0.05} nats, a 5\% relative reduction in the geometric-mean probability assigned to true tail-class labels, unless the analogous tail-NLL $\sigma_{\mathrm{seed}}$ exceeds \num{0.025}, in which case it is raised to $2\sigma_{\mathrm{seed}}$; here it does ($\sigma_{\mathrm{seed}}=0.09119$, CAMELYON16), so the calibration threshold is raised accordingly. A comparison unit entering through the discrimination gate is evaluated for balanced-accuracy recovery; one entering through the calibration gate is evaluated for tail-NLL recovery under the method-specific score semantics of Table~\ref{tab:score-variants}; it may enter through both.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.27\linewidth}Xl@{}}
    ...
    Deficit gate thresholds & $D_{\mathrm{BA}}\geq\num{0.01443}$; $D_{\mathrm{cal}}\geq\num{0.18237}$ nats; both with CI excluding 0 & \ref{sec:recovery} \\
    \bottomrule
  \end{tabularx}
