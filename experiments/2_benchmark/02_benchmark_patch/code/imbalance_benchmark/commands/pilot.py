...
def _pilot_setup(
    ...
    grp = train_df.groupby("cancer_type")
    available = (
        {
        ...
        else grp[unit_col].nunique().to_dict()
    )
    pats, slds = grp["case_id"].nunique().to_dict(), grp["slide_id"].nunique().to_dict()
    support = {
        cls: {"patients": int(pats.get(cls, 0)), "slides": int(slds.get(cls, 0))}
        for cls in classes
    }
