...
\label{sec:design}
Interpretability depends on separating the intended intervention from everything else. The benchmark therefore freezes three layers of the comparison. Task definition fixes the data source, prediction target, and frozen representation. Evidence design fixes the split-unit partition and total controlled support. Model comparison fixes the natural validation and test cohorts and the training budget. Only class allocation within training support changes. Figure~\ref{fig:study-logic} follows that contrast from construction to interpretation.
...
\begin{figure}[H]
  ...
  \end{tikzpicture}
  \caption{Logic of the benchmark. RQ1 concerns the damage contrast, RQ2 concerns gated recovery, and RQ3 concerns variation among those controlled comparisons.}
...
\noindent The balanced, moderate, and severe conditions contain the same total training support $T$: patches for patch classification and slides for MIL. The shared $T$ is selected jointly with severity, not from size alone: among candidate totals satisfying the independent-support and contribution-cap constraints, construction retains the largest one whose realized moderate and severe ratios both lie within 10\% of $\rho_{\mathrm{target}}=10$ and $\rho_{\mathrm{target}}=100$ (that is, $\rho_{\mathrm{achieved}}\in[9,11]$ and $\rho_{\mathrm{achieved}}\in[90,110]$ respectively) for every locked tail assignment, confirmed against the true fixed-pool construction. Choosing $T$ from size alone could flatten the intended contrast, and this tolerance keeps both controlled conditions recognizably close to their nominal severities rather than only as separated as the data allow. Appendix~\ref{app:construction} specifies the tolerance search and its feasibility rules.
...
\begin{equation}
  ...
  \label{eq:kish}
\end{equation}
\noindent translates unequal case weights into the equivalent number of equally weighted split units. It decreases as weight concentrates. The preflight also records each split/class cell's number of contributing units and its largest unit-weight share. If the 2.5th percentile of $n_{\mathrm{Kish}}$ across replicates is below five, one unit supplies more than 50\% of a class's weight in more than 5\% of replicates, or a split/class cell has fewer than five contributing units, inference for that dataset--regime is designated descriptive before training. A percentile rather than the minimum is used because the minimum across replicates is an extreme order statistic: its value falls as the replicate count rises, so a floor placed on it would encode the resampling budget rather than the support the cell actually carries. The diagnostic report and resampling seed are frozen with the analysis manifest.
...
\end{equation}
updates, corresponding to 30 reference passes through $T$ examples under ordinary sampling. Balanced sampling, replacement, synthetic bags, and dual loaders do not change $U$. RankMix receives $U$ teacher updates and, after reinitialization, $U$ student updates. cRT receives $U$ stage-one updates and $\lceil0.2U\rceil$ stage-two classifier updates. MDE performs $U$ joint updates, each consuming its prespecified natural and balanced minibatches. The natural anchor's CE run uses the same formula with its own support and is descriptive. Processed examples, unique-example exposures, and updates are all reported. Each run stores validation metrics at fixed update intervals; the interval is scaled per dataset so the number of stored validation checkpoints is comparable across the widely differing controlled supports $T$, and the resolved value is recorded with every run. The selected checkpoint maximizes natural-validation balanced accuracy, with macro F1 and then lower NLL as deterministic tie-breakers. Test predictions are produced once from that locked checkpoint.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\linewidth,keepaspectratio]{assets/cls_imb_benchmark.png}
  \caption{Operational workflow of the controlled class-imbalance benchmark. The validation search is split into a self-contained phase, in which every candidate is fitted from scratch and needs no other method's result, and a CE-inherited phase, in which post-hoc logit adjustment and cRT reuse a condition's selected CE configuration and so cannot begin until that condition's CE search has resolved.}
