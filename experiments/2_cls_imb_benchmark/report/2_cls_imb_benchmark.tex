...
\label{sec:data}
Table~\ref{tab:datasets} defines the prediction example and label in each available dataset--regime. A \emph{patch-level example} is one eligible $256\times256$ crop and receives the label specified in the second column. A \emph{bag-level example} is one WSI represented by its eligible frozen patch embeddings under the fixed instance cap and receives one label for the entire WSI. A patient or case is assigned to exactly one partition before any examples are selected. Where one patient contributes multiple slides, all slides follow the patient assignment. A fixed eligibility pipeline removes unusable samples before splitting and is identical across imbalance conditions.
