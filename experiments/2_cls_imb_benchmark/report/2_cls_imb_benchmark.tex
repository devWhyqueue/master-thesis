...
\label{sec:analysis}
Evaluation proceeds from description to explanation. It first measures which classes are predicted correctly, then reports PANDA-specific ordinal error and examines probability quality. It next determines whether imbalance caused a deficit, quantifies uncertainty at the patient level, and tests recovery while accounting for multiple comparisons. Finally, RQ3 explores why damage and recovery differ across controlled comparisons. All endpoints, decision rules, and inferential families are fixed before test evaluation.
...
Because checkpoints and hyperparameters are selected for natural-validation balanced accuracy, all probability-quality results describe discrimination-selected models rather than models optimized for calibration. The output semantics and prior corrections of individual methods are defined in the companion methods report rather than in this endpoint section \citep{queisler2026imbalanceMethods}.
...
\subsection{Uncertainty from patient resampling}
A \emph{bootstrap} approximates sampling uncertainty by repeatedly constructing hypothetical test cohorts from the observed patients. Sampling is performed \emph{with replacement}: within one replicate, a patient can be selected more than once or not selected at all. The metric is recomputed for every replicate, and the spread of those values shows how much the result could change under a similar patient sample. Patients---not patches---are resampled because patches from one patient are correlated.

\paragraph{Label-only preflight.}
Before model training, \num{10000} bootstrap replicates are constructed using only patient identifiers, split membership, and class labels. \emph{Label-only} means that no predictions, performance values, or method identities are used; these replicates test whether the planned resampling scheme is valid before results can influence it. The count of \num{10000} is a prespecified Monte Carlo budget. It places about 250 replicates in each 2.5\% tail of a percentile interval, reducing simulation noise in the interval endpoints while remaining inexpensive at this label-only stage.

A \emph{stratum} is a group of patients with the same observed contribution pattern. Here, that pattern records how many examples a patient contributes to each class in each test-split appearance. Patients are resampled only against other patients in the same stratum. This preserves the observed split--class structure, including patch targets for which one patient can contribute several classes. A selected patient's multiplicity is applied to all of that patient's slides and patches in every split. When patient and slide coincide, slides are the resampling units. Figure~\ref{fig:bootstrap-workflow} summarizes the workflow.
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[
    ...
    \node[fixed] (identity) {Observed test identities\\patient, split, and class};
    \node[fixed, right=0.35cm of identity] (strata) {Group patients by\\contribution stratum};
    \node[sampled, right=0.35cm of strata] (resample) {Resample patients\\with replacement};
  ...
  \end{tikzpicture}
  \caption{Patient-level bootstrap workflow. The label-only preflight executes the identity, stratification, and resampling steps before predictions exist. After model fitting, the same paired patient multiplicities are applied to every matched run.}
  \label{fig:bootstrap-workflow}
\end{figure}

\noindent The preflight verifies that every original split--class cell remains represented, every split-level metric is computable, and one patient receives the same multiplicity across all appearances. Failure of any check stops the analysis rather than causing a replicate to be silently discarded.

Resampling with replacement can still produce a replicate dominated by a few repeatedly selected patients. If patient $i$ has class-specific bootstrap weight $v_i$, the \emph{Kish effective patient count}
\begin{equation}
  n_{\mathrm{Kish}}=\frac{\left(\sum_i v_i\right)^2}{\sum_i v_i^2}.
  \label{eq:kish}
\end{equation}
\noindent translates unequal patient weights into the equivalent number of equally weighted patients. It decreases as weight concentrates. The preflight also records the number of unique patients and the largest patient weight by split and class. If $n_{\mathrm{Kish}}<5$ in any replicate, or one patient supplies more than 50\% of a class's weight in more than 5\% of replicates, inference for that dataset--regime is designated descriptive before training. The diagnostic report and resampling seed are frozen with the analysis manifest.

\paragraph{Intervals after model fitting.}
The three patient splits are fixed design repetitions and are averaged with equal weight; they are not treated as three independent cohorts. Tail assignments remain separate unless an assignment-averaged estimand is explicitly reported. Within each patient-bootstrap replicate, the five matched confirmation-seed indices are also resampled as paired blocks and averaged before the splits are combined. This captures algorithmic variation without pretending that five model initializations create five additional patient cohorts.

Balanced CE, imbalanced CE, and each mitigation method receive the same patient multiplicities and confirmation-seed draws. The complete deficit and recovery fraction, including numerator and denominator, is recomputed in each replicate. The reported 95\% percentile interval spans the 2.5th to 97.5th percentiles of the resulting distribution. Patches are never bootstrapped independently.
...
The raw two-sided $p$-value comes from a paired patient-block permutation test. This test represents the null hypothesis of no method difference by repeatedly swapping the method and imbalanced-CE labels within a patient. The same swap is used for all of that patient's slides, patches, split appearances, and matched confirmation seeds, preserving the paired design. All possible swaps are enumerated when feasible. Otherwise, \num{100000} random swaps are used with a plus-one correction; this gives a minimum attainable Monte Carlo $p$-value of approximately $10^{-5}$ and avoids reporting a simulated value of zero.
...
\begin{table}[ht]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.23\linewidth}>{\raggedright\arraybackslash}p{0.49\linewidth}X@{}}
    ...
    Minimum independent support & How many distinct patients or slides support the scarcest class? & One-predictor sensitivity \\
    Effective patch support & How much patch support remains after accounting for within-patient redundancy? & Patch-only sensitivity \\
...
Raw patch count can overstate evidence when many patches come from the same patient. Unique patient and slide counts therefore remain the primary support measures. As a secondary sensitivity analysis, within-patient redundancy is summarized by the intraclass correlation (ICC) of a cross-fitted class margin. An ICC near zero means that patches from the same patient are no more alike than patches from different patients; an ICC near one indicates strong redundancy.
