...
\noindent Because every larger pilot level extends the preceding prefix, adjacent levels are paired within an ordering and evaluated on the same natural-validation cohort. The smallest level whose next increment changes mean balanced accuracy by less than \num{0.01} and every class recall by less than \num{0.02} in all three orderings is the \emph{stability floor}; if none qualifies, the largest candidate is used.\footnote{No pilot level is exempt from the \num{10}\% split-unit or \num{5}\% slide contribution cap. A candidate level is dropped from the curve, for every ordering, only when no quota satisfies both caps at that level for every construction seed---for example, a \num{5}\% slide cap requires at least 20 contributing slides regardless of quota, so a class whose split units mostly contribute a single eligible slide can leave levels below 20 infeasible. Dropped levels are recorded alongside the requested candidates in the frozen pilot report.}
