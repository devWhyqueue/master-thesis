...
\noindent Because every larger pilot level extends the preceding prefix, adjacent levels are paired within an ordering and evaluated on the same natural-validation cohort. The smallest level whose next increment changes mean balanced accuracy by less than \num{0.01} and every class recall by less than \num{0.02} in all three orderings is the \emph{stability floor}; if none qualifies, the largest candidate is used.\footnote{The five-unit pilot is the sole cap exception. With only five contributing split units, at least one must supply at least \num{20}\% of a class, so a \num{10}\% cap is mathematically impossible; a \num{5}\% slide cap also requires at least 20 contributing slides. The patch pilot therefore uses five split units with equal quotas, and the MIL pilot uses five slides from five split units. This diagnostic pilot level is never a definitive training allocation.}
...
\label{app:overview}
Figure~\ref{fig:benchmark-overview} summarizes the benchmark from dataset selection and frozen feature extraction through controlled imbalance, mitigation, and evaluation.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth,height=0.72\textheight,keepaspectratio]{assets/cls_imb_benchmark.png}
  \caption{Accessible overview of the controlled class-imbalance benchmark. The workflow holds the evidence, representation, partitions, and training budget fixed while varying training class allocation or the mitigation method, then measures imbalance-induced damage and mitigation recovery on natural held-out data.}
