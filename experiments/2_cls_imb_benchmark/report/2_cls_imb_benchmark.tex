...
\title{Controlled Evaluation of Class-Imbalance Mitigation Signals\\
in Histopathology Patch Classification}
...
\noindent Class imbalance is difficult to isolate in computational pathology because raw sample counts can hide a second shortage: few labelled patients or slides behind many informative patches. This benchmark asks a controlled counterfactual question, and a second question about which signal a mitigation method should read to answer it: how would the same prediction task change if one fixed training budget were distributed evenly across classes rather than concentrated in head classes, and does a method that reads the resulting shortage's own signal recover more of the resulting damage than one that reads a different signal? CAMELYON16, TCGA-UT, BRACS, and PANDA are studied in patch classification using frozen Virchow2 features. Patient- or case-disjoint partitions, natural validation and test cohorts, model family, and optimizer-update budgets remain fixed while only training support is redistributed among size-matched balanced, moderate, and severe conditions. Balanced accuracy and tail-group macro negative log likelihood measure discrimination and probability quality. Mitigation recovery is interpreted only after a prespecified gate establishes a meaningful imbalance-induced deficit. A five-member family of methods --- one per signal, all a mean-one class weight on unmodified cross-entropy --- makes the signal contrast interpretable at a fixed intervention level. Exploratory analyses then relate damage and recovery to achieved severity, support--difficulty alignment, and independent support. The design generalizes to WSI-level multiple-instance learning, but this protocol is scoped to patch classification; a companion protocol may cover the WSI-bag regime.
\par
\endgroup
\end{abstract}
\vspace*{\fill}
...
Class imbalance is usually described by counting examples per class. In pathology, that count can be deceptive. Thousands of patches may come from only a few patients, so a class with abundant patches can still rest on a thin pool of independent cases. Patch selection draws round-robin from a per-class case pool under a patient contribution cap, so a severe patch count and the number of distinct cases behind it need not track each other: a class can be patch-rich yet case-poor, or the reverse. Frequent classes then dominate the empirical objective, while rare classes may provide too little independent evidence for stable decision boundaries or reliable probabilities \citep{saini2023tacklingClass,zhang2023deepLong,mosquera2024classImbalance}.
...
A second, orthogonal question follows from the first: imbalance can create more than one kind of shortage --- of raw prevalence, of nominal or independent support, of learnable signal in difficult classes, of representational diversity --- and a mitigation method that reads one signal need not help when a different shortage is the one actually present. The companion methods report places the tested roster on this signal axis \citep{queisler2026imbalanceMethods}; this benchmark is where that axis is tested empirically, not only defined.

These two questions create a sequence of three research questions:
\begin{description}
  \item[RQ1.] \textbf{Which shortage does controlled allocation create, and what does it damage?} At fixed task, split-unit partition, encoder, training-set size, and evaluation set, how does increasing training imbalance affect tail-class discrimination and calibration in patch classification, and what realized signal profile --- prevalence $\pi_c$, nominal support $n_c$ or $E_c(\beta)$, independent support $G_c$ or $N_{\mathrm{eff},c}$, difficulty $d_c$, diversity $S'_c$ --- does each controlled condition present?
  \item[RQ2.] \textbf{Does mitigation recover more when the signal it reads matches the shortage present?} Which mitigation methods recover performance lost specifically to training imbalance, and does a method's recovery depend on whether the signal it reads corresponds to the shortage RQ1 identified? The roster is grouped by signal, and five methods --- weighted CE, class-balanced CE, independent-support weighted CE, pilot-difficulty weighted CE, and semantic-scale weighted CE --- share one intervention level, a mean-one class weight on unmodified CE, so a difference among them is attributable to the signal rather than the mechanism.
...
\end{description}
RQ1 first establishes whether a recoverable problem exists, and which signal(s) characterize it. RQ2 is evaluated only against that demonstrated deficit, and asks whether reading the right signal matters. RQ3 then asks why the size of the deficit and available headroom differ; it is an exploratory association analysis, not another method ranking. RQ1 gates RQ2, and RQ3 is exploratory throughout. This protocol is scoped to patch classification: the round-robin case-pool sampling above only decouples nominal from independent support because a patch redistributes evidence inside a fixed case pool, whereas redistributing labelled slides moves both together. A WSI-bag protocol testing the same signal axis under that different regime may follow; it is out of scope here. Dataset provenance and detailed method objectives remain in the companion reports \citep{queisler2026datasets,queisler2026imbalanceMethods}. The next section turns this sequence into the benchmark's organizing logic.
...
\noindent One \emph{comparison unit} is a dataset, imbalance severity, and tail assignment evaluated together. A \emph{deficit gate} is a prespecified rule that asks whether imbalance caused enough damage, with sufficient precision, to make a recovery ratio interpretable. \emph{Recovery} is the fraction of that demonstrated deficit regained by a mitigation method. These are analysis concepts, not model components.

Every quantity that this logic holds fixed is prespecified before the first definitive fit. Table~\ref{tab:locked} collects them with a pointer to the specification that derives each one, so the sections that follow can argue about the intervention rather than restate its settings.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.27\linewidth}Xl@{}}
    ...
    Discrimination gate & $D_{\mathrm{BA}}\geq\num{0.02}$, paired CI excludes 0 & \ref{sec:recovery} \\
    Calibration gate & $D_{\mathrm{cal}}\geq\num{0.05}$ nats, paired CI excludes 0 & \ref{sec:recovery} \\
    Bootstrap replicates & \num{10000}, split-unit-clustered & \ref{app:endpoints} \\
    \bottomrule
  \end{tabularx}
  \caption{Quantities locked before the first definitive fit, with the section or appendix that specifies each. Only class allocation within the shared training support $T$ varies across conditions.}
  \label{tab:locked}
\end{table}
...
\label{sec:data}
This protocol covers one prediction regime: patch classification predicts a label for one directly labelled image crop. The companion methods report also defines a WSI-bag regime, in which one label is predicted for a complete slide represented as a bag of patch embeddings and patches are instances rather than independently labelled examples \citep{queisler2026imbalanceMethods}; that regime is out of scope here and is left to a possible companion protocol. The terms \emph{patch} and \emph{tile} both denote a $256\times256$ crop, but \emph{patch} is used throughout.
...
\begin{table}[ht]
  ...
  \begin{tabularx}{\linewidth}{@{}l>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X@{}}
    ...
    \bottomrule
  \end{tabularx}
  \caption{Dataset-specific definitions of patch-level examples. TCGA-UT's eligible target set omits one of the 32 annotated cancer types; Appendix~\ref{app:construction} states which class and why. The companion methods report defines the corresponding WSI-bag target for each dataset, which this protocol does not evaluate \citep{queisler2026imbalanceMethods}.}
  \label{tab:datasets}
\end{table}
...
\label{sec:methods}
Table~\ref{tab:roster} lists only the methods evaluated in this benchmark, grouped by the mitigation signal each one reads rather than by intervention family, matching Table~\ref{tab:signal-intervention} of the companion methods report so the two stay comparable at a glance. CE is the size-matched no-mitigation reference. The roster is the patch-applicable subset of that report's full taxonomy: the methods report roster is deliberately broader than this protocol's roster, since it also defines a WSI-bag regime this protocol does not evaluate; a WSI protocol may follow covering that subset. The roster is fitted in the balanced, moderate, and severe conditions, which alone supply the paired contrast a mitigation claim requires; the natural anchor carries the CE reference only. Method objectives, tested adaptations, ablations, defaults, and fidelity qualifications are defined exclusively in the companion methods report \citep{queisler2026imbalanceMethods}.
...
\begin{table}[ht]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.20\linewidth}>{\raggedright\arraybackslash}p{0.30\linewidth}X@{}}
    ...
    \bottomrule
  \end{tabularx}
  \caption{Prespecified patch-regime method roster, grouped by mitigation signal as in Table~\ref{tab:signal-intervention} of the companion methods report, with intervention level retained as a second column so the two axes stay visibly orthogonal.}
  \label{tab:roster}
\end{table}
...
PANDA's patch target is binary (cancer versus benign), not the biopsy's ordinal ISUP grade; that grade is a WSI-level label outside this protocol's patch prediction unit (Table~\ref{tab:datasets}), so no ordinal endpoint is reported here. A patch-classification route to biopsy-level ordinal grading would require a new aggregation rule from patch-level cancer probability to a grade estimate, which this protocol does not define; it is left to future work or a WSI-bag protocol, where the ISUP grade is the bag's own target.
...
\begin{table}[ht]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.30\linewidth}>{\raggedright\arraybackslash}p{0.24\linewidth}X@{}}
    ...
    \bottomrule
  \end{tabularx}
  \caption{Hierarchical selection and contribution caps. Every manifest reports per-class split-unit, slide, and patch counts, maximum contributions, and the fraction of the eligible independent pool retained.}
