#!/bin/bash
# Build per-epoch synthetic SQFS files (e10, e25, e50) x 3 seeds.
# Chain baseline progan-train (epoch=25) after e25 builds complete.
set -euo pipefail
cd /home/yannik.qu/master-thesis/experiments/2_cls_imb_benchmark/tcga_ut
...
build_epoch() {
  ...
  sbatch --parsable \
    ...
    --mem=64G \
    --array=0-2 \
    --job-name=tcga-ut-synth-sqfs-e${epoch} \
    --output=logs/build-synthetic-sqfs-e${epoch}-%A-%a.out \
    --error=logs/build-synthetic-sqfs-e${epoch}-%A-%a.err \
    code/hydra/job.sbatch
}

e10=$(build_epoch 10); echo "Build e10: ${e10}"
e25=$(build_epoch 25); echo "Build e25: ${e25}"
e50=$(build_epoch 50); echo "Build e50: ${e50}"
...
train_job=$(sbatch --parsable \
  ...
  code/hydra/job.sbatch)
echo "Train (seeds 0-2):  ${train_job} (after e25 ${e25})"
