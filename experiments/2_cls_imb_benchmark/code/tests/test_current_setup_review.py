...
def test_pilot_construction_and_initialization_seeds_are_separate(
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: (observed.append(kwargs["initialization_seed"]) or (0.5, [0.5])),
    )
...
def test_bag_instance_cap_uses_the_frozen_selection_seed(tmp_path: Path) -> None:
    ...
    first, _ = BagFeatureDataset(manifest, max_instances=4, instance_selection_seed=1)[0]
    repeat, _ = BagFeatureDataset(manifest, max_instances=4, instance_selection_seed=1)[0]
    second, _ = BagFeatureDataset(manifest, max_instances=4, instance_selection_seed=2)[0]
...
def test_crt_freezes_the_full_representation_in_deterministic_eval_mode(
    ...
    assert all(not parameter.requires_grad for module in frozen for parameter in module.parameters())
    assert all(not module.training for module in frozen)
    assert all(parameter.requires_grad for parameter in model.classifier.parameters()) if is_mil else all(
        parameter.requires_grad for parameter in model.net[-1].parameters()
    )
...
def test_rq3_icc_margin_uses_the_fixed_intrinsic_reference(
    ...
    monkeypatch.setattr(
        ...
        lambda path, *_: pd.DataFrame({"case_id": ["a", "b"], "slide_id": ["s1", "s2"]}),
    )
    monkeypatch.setattr(
